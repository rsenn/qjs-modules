...
export function Console(...args) {
  ...
  if(typeof out == 'object' && out != null) {
    if(typeof out.puts == 'function') {
    } else if(typeof out == 'object') {
      opts = 'inspectOptions' in out ? out : { inspectOptions: out };
      out = std.out;
    }
  }
  ...
  const consoleDimensions = (fd = termFd) => {
    let size;
    try {
      size = os.ttyGetWinSize(fd);
    } catch(err) {}
    return Array.isArray(size) ? size : [undefined, undefined];
  };

  const isTerminal = os.isatty(termFd);

  const defaultBreakLength = (isTerminal && consoleDimensions()[0]) || env.COLUMNS || Infinity;
  const defaultOptions = {
    depth: 2,
    colors: isTerminal,
    stringBreakNewline: false,
    maxArrayLength: isTerminal ? (consoleDimensions()[1] ?? 24) - 6 : Infinity,
  ...
  const printFunction = out => (typeof out == 'function' ? out : text => out.puts(text));

  /*  if(out) out = printFunction(out);*/
