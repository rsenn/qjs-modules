 ...
 */
export function WindowTestEnvironment() {
  this.name_counter = 0;
  ...
  this.message_functions = {
    start: [
      ...
      function(properties) {
        this_obj._dispatch('start_callback', [properties], { type: 'start', properties: properties });
      },
    ],
    ...
    test_state: [
      ...
      function(test) {
        this_obj._dispatch('test_state_callback', [test], { type: 'test_state', test: test.structured_clone() });
      },
    ],
    result: [
      add_result_callback,
      ...
      function(test) {
        ...
        this_obj._dispatch('result_callback', [test], { type: 'result', test: test.structured_clone() });
      },
    ],
    completion: [
      ...
      function(tests, harness_status, asserts) {
        const cloned_tests = map(tests, function(test) {
  ...
  on_event(window, 'message', function(event) {
    if(event.data && event.data.type === 'getmessages' && event.source) {
      ...
      // this environment.
      for(const i = 0; i < this_obj.dispatched_messages.length; ++i) {
...
WindowTestEnvironment.prototype._dispatch = function(selector, callback_args, message_arg) {
  ...
  this._forEach_windows(function (w, same_origin) {
    if(same_origin) {
      try {
        const has_selector = selector in w;
      } catch(e) {
...
WindowTestEnvironment.prototype.setup_messages = function(new_events) {
  ...
  forEach(settings.message_events, function(x) {
    const current_dispatch = this_obj.message_events.indexOf(x) !== -1;
    const new_dispatch = new_events.indexOf(x) !== -1;
...
WindowTestEnvironment.prototype.next_default_test_name = function() {
  const suffix = this.name_counter > 0 ? ' ' + this.name_counter : '';
  this.name_counter++;
...
WindowTestEnvironment.prototype.test_timeout = function() {
  ...
  for(const i = 0; i < metas.length; i++) {
 ...
 */
export function WorkerTestEnvironment() {
  this.name_counter = 0;
...
WorkerTestEnvironment.prototype._dispatch = function(message) {
  ...
  for(const i = 0; i < this.message_ports.length; ++i) {
...
WorkerTestEnvironment.prototype._add_message_port = function(port) {
  ...
  for(const i = 0; i < this.message_list.length; ++i) {
 ...
 */
export function SharedWorkerTestEnvironment() {
  WorkerTestEnvironment.call(this);
  const this_obj = this;
 ...
 */
export function ServiceWorkerTestEnvironment() {
  WorkerTestEnvironment.call(this);
  this.all_loaded = false;
 ...
 */
export function ShadowRealmTestEnvironment() {
  WorkerTestEnvironment.call(this);
  this.all_loaded = false;
  this.on_loaded_callback = null;
}
 ...
 */
export function ShellTestEnvironment() {
  this.name_counter = 0;
  this.all_loaded = false;
  this.on_loaded_callback = null;
...
const test_environment = create_test_environment();
...
const seen_func_name = Object.create(null);
...
function get_test_name(func, name) {
  ...
  if(func) {
    ...
    const arrow = func_code.match(/^\(\)\s*=>\s*(?:{(.*)}\s*|(.*))$/);

    // Check for JS line separators
    if(arrow !== null && !/[\u000A\u000D\u2028\u2029]/.test(func_code)) {
      const trimmed = (arrow[1] !== undefined ? arrow[1] : arrow[2]).trim();
 ...
 */
export function test(func, name, properties) {
  if(tests.promise_setup_called) {
    tests.status.status = tests.status.ERROR;
    tests.status.message = '`test` invoked after `promise_setup`';
    tests.complete();
  }
  const test_name = get_test_name(func, name);

  const test_obj = new Test(test_name, properties);

  const value = test_obj.step(func, test_obj, test_obj);

  if(value !== undefined) {
    const msg = 'Test named "' + test_name + '" passed a function to `test` that returned a value.';
 ...
 */
export function async_test(func, name, properties) {
  if(tests.promise_setup_called) {
    tests.status.status = tests.status.ERROR;
  ...
  if(func) {
    ...
    if(value !== undefined) {
      const msg = 'Test named "' + test_name + '" passed a function to `async_test` that returned a value.';
 ...
 * given file and must be invariant between runs.
 */
export function promise_test(func, name, properties) {
  if(typeof func !== 'function') {
    properties = name;
    name = func;
    func = null;
  ...
  const test_name = get_test_name(func, name);
  const test = new Test(test_name, properties);
 ...
 *
 */
export function promise_rejects_dom(test, type, promiseOrConstructor, descriptionOrPromise, maybeDescription) {
 ...
 *
 */
export function EventWatcher(test, watchedNode, eventTypes, timeoutPromise) {
  if(typeof eventTypes == 'string') {
    eventTypes = [eventTypes];
  }

  const waitingFor = null;

  // This is null unless we are recording all events, in which case it
  // will be an Array object.
  const recordedEvents = null;
  ...
  const eventHandler = test.step_func(function (evt) {
    ...
    const resolveFunc = waitingFor.resolve;
    waitingFor = null;
  ...
  for(const i = 0; i < eventTypes.length; i++) {
 ...
 *
 */
export function setup(func_or_properties, maybe_properties) {
  const func = null;
  const properties = {};
 ...
 *
 */
export function promise_setup(func, properties = {}) {
  if(typeof func !== 'function') {
 ...
 * `func(..testArgs)`.
 */
export function generate_tests(func, args, properties) {
 ...
 */
export function on_event(object, event, callback) {
 ...
 * ``timeout_multiplier``.
 */
export function step_timeout(func, timeout) {
...
function is_node(object) {
  ...
  try {
    const has_node_properties = 'nodeType' in object && 'nodeName' in object && 'nodeValue' in object && 'childNodes' in object;
  } catch(e) {
 ...
 */
export function format_value(val, seen) {
  if(!seen) {
    seen = [];
  }
  ...
  switch (typeof val) {
    case 'string':
      val = val.replace(/\\/g, '\\\\');
      for(let p in replacements) {
        const replace = '\\' + replacements[p];
      ...
      if(is_node(val)) {
        switch (val.nodeType) {
          case Node.ELEMENT_NODE:
            const ret = '<' + val.localName;
            for(const i = 0; i < val.attributes.length; i++) {
...
export function assert_equals(actual, expected, description) {
  ...
  if(typeof actual != typeof expected) {
    assert(false, 'assert_equals', description, 'expected (' + typeof expected + ') ${expected} but got (' + typeof actual + ') ${actual}', { expected: expected, actual: actual });
    return;
  }
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_in_array(actual, expected, description) {
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_object_equals(actual, expected, description) {
  assert(typeof actual === 'object' && actual !== null, 'assert_object_equals', description, 'value is ${actual}, expected object', { actual: actual });
  ...
  function check_equal(actual, expected, stack) {
    ...
    let p;
    for(p in actual) {
      assert(expected.hasOwnProperty(p), 'assert_object_equals', description, 'unexpected property ${p}', { p: p });
      ...
      if(typeof actual[p] === 'object' && actual[p] !== null) {
        ...
        assert(same_value(actual[p], expected[p]), 'assert_object_equals', description, 'property ${p} expected ${expected} got ${actual}', { p: p, expected: expected[p], actual: actual[p] });
      }
    }
    for(p in expected) {
...
export function assert_array_equals(actual, expected, description) {
  ...
  assert(typeof actual === 'object' && actual !== null && 'length' in actual, 'assert_array_equals', description, 'value is ${actual}, expected array', { actual: actual });
  ...
  for(const i = 0; i < actual.length; i++) {
    assert(
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_array_approx_equals(actual, expected, epsilon, description) {
  /*
   * Test if two primitive arrays are equal within +/- epsilon
   */
  assert(actual.length === expected.length, 'assert_array_approx_equals', description, 'lengths differ, expected ${expected} got ${actual}', { expected: expected.length, actual: actual.length });
  ...
  for(const i = 0; i < actual.length; i++) {
    ...
    assert(typeof actual[i] === 'number', 'assert_array_approx_equals', description, 'property ${i}, expected a number but got a ${type_actual}', { i: i, type_actual: typeof actual[i] });
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_approx_equals(actual, expected, epsilon, description) {
  /*
   * Test if two primitive numbers are equal within +/- epsilon
   */
  assert(typeof actual === 'number', 'assert_approx_equals', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });
...
export function assert_less_than(actual, expected, description) {
  ...
  assert(actual < expected, 'assert_less_than', description, 'expected a number less than ${expected} but got ${actual}', { expected: expected, actual: actual });
}
...
export function assert_greater_than(actual, expected, description) {
   ...
   */
  assert(typeof actual === 'number', 'assert_greater_than', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });

  assert(actual > expected, 'assert_greater_than', description, 'expected a number greater than ${expected} but got ${actual}', { expected: expected, actual: actual });
}
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_between_exclusive(actual, lower, upper, description) {
  /*
   * Test if a primitive number is between two others
   */
  assert(typeof actual === 'number', 'assert_between_exclusive', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });
...
export function assert_less_than_equal(actual, expected, description) {
   ...
   */
  assert(typeof actual === 'number', 'assert_less_than_equal', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });

  assert(actual <= expected, 'assert_less_than_equal', description, 'expected a number less than or equal to ${expected} but got ${actual}', { expected: expected, actual: actual });
}
...
export function assert_greater_than_equal(actual, expected, description) {
   ...
   */
  assert(typeof actual === 'number', 'assert_greater_than_equal', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });

  assert(actual >= expected, 'assert_greater_than_equal', description, 'expected a number greater than or equal to ${expected} but got ${actual}', { expected: expected, actual: actual });
}
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_between_inclusive(actual, lower, upper, description) {
  /*
   * Test if a primitive number is between to two others or equal to either of them
   */
  assert(typeof actual === 'number', 'assert_between_inclusive', description, 'expected a number but got a ${type_actual}', { type_actual: typeof actual });
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_regexp_match(actual, expected, description) {
  /*
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_class_string(object, class_string, description) {
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_own_property(object, property_name, description) {
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_not_own_property(object, property_name, description) {
...
function _assert_inherits(name) {
  return function(object, property_name, description) {
    ...
    assert(!object.hasOwnProperty(property_name), name, description, 'property ${p} found on object expected in prototype chain', { p: property_name });
 ...
 * @param {string} [description] - Description of the condition being tested.
 */
export function assert_readonly(object, property_name, description) {
  const initial_value = object[property_name];
  try {
    //Note that this can have side effects in the case where
    //the property has PutForwards
    object[property_name] = initial_value + 'a'; //XXX use some other value here?
    assert(same_value(object[property_name], initial_value), 'assert_readonly', description, 'changing property ${p} succeeded', { p: property_name });
  } finally {
...
function assert_throws_js_impl(constructor, func, description, assertion_type) {
  try {
    ...
    assert(typeof e === 'object', assertion_type, description, '${func} threw ${e} with type ${type}, not an object', { func: func, e: e, type: typeof e });

    assert(e !== null, assertion_type, description, '${func} threw null, not an object', { func: func });
 ...
 *
 */
export function assert_throws_dom(type, funcOrConstructor, descriptionOrFunc, maybeDescription) {
...
function assert_throws_dom_impl(type, func, description, assertion_type, constructor) {
  try {
    ...
    assert(typeof type == 'number' || typeof type == 'string', assertion_type, description, '${type} is not a number or string', { type: type });
    ...
    assert(e.constructor === constructor, assertion_type, description, '${func} threw an exception from the wrong global', { func });
  }
}
 ...
 * @param {string} [description] Error description for the case that the error is not thrown.
 */
export function assert_throws_exactly(exception, func, description) {
...
function assert_throws_exactly_impl(exception, func, description, assertion_type) {
  try {
    ...
    assert(same_value(e, exception), assertion_type, description, '${func} threw ${e} but we expected it to throw ${exception}', { func: func, e: e, exception: exception });
  }
}
 ...
 */
export function assert_any(assert_func, actual, expected_array, ...args) {
  const errors = [];
  const passed = false;
 ...
 */
export function assert_implements(condition, description) {
 ...
 * @param {string} [description] Error description for the case that the condition is not truthy.
 */
export function assert_implements_optional(condition, description) {
  if(!condition) {
 ...
 *
 */
export function Test(name, properties) {
  (globalThis.tests ??= []).push(this);
...
Test.prototype.step = function(func, this_obj) {
  ...
  try {
    ...
    const status = e instanceof OptionalFeatureUnsupportedError ? this.PRECONDITION_FAILED : this.FAIL;
    const message = String(typeof e === 'object' && e !== null ? e.message : e);
    const stack = e.stack ? e.stack : null;
...
Test.prototype.step_wait_func = function(cond, func, description, timeout = 3000, interval = 100) {
  const timeout_full = timeout * tests.timeout_multiplier;
  const remaining = Math.ceil(timeout_full / interval);
  const test_this = this;

  const wait_for_inner = test_this.step_func(() => {
    if(cond()) {
      func();
    } else {
      if(remaining === 0) {
...
function cleanup_done(test, errors, bad_value_count) {
  if(errors.length || bad_value_count) {
    const total = test._user_defined_cleanup_count;
    ...
    if(bad_value_count) {
      const type = test._is_promise_test ? 'non-thenable' : 'non-undefined';
 ...
 */
export function RemoteTest(clone) {
  const this_obj = this;
 ...
 * MessageEvents.
 */
export function RemoteContext(remote, message_target, message_filter) {
  this.running = true;
  this.started = false;
  ...
  this.message_handler = function(message) {
    const passesFilter = !message_filter || message_filter(message);
...
RemoteContext.prototype.test_state = function(data) {
  const remote_test = this.tests[data.test.index];
  if(!remote_test) {
 ...
 */
export function AssertRecord(test, assert_name, args = []) {
...
Tests.prototype.set_timeout = function() {
  if(global_scope.clearTimeout) {
    const this_obj = this;
    //console.log('timeout_id', this.timeout_id);
    if(this.timeout_id !== null) clearTimeout(this.timeout_id);
    if(this.timeout_length !== null) {
      this.timeout_id = setTimeout(function () {
        this_obj.timeout();
...
Tests.prototype.complete = function() {
  ...
  const this_obj = this;
  const all_complete = function() {
    this_obj.phase = this_obj.phases.COMPLETE;
    this_obj.notify_complete();
  };
  const incomplete = filter(this.tests, function(test) {
...
function sanitize_unpaired_surrogates(str) {
  return str.replace(/([\ud800-\udbff]+)(?![\udc00-\udfff])|(^|[^\ud800-\udbff])([\udc00-\udfff]+)/g, function(_, low, prefix, high) {
    const output = prefix || ''; // prefix may be undefined
...
function sanitize_all_unpaired_surrogates(tests) {
  forEach(tests, function(test) {
    const sanitized = sanitize_unpaired_surrogates(test.name);
...
Tests.prototype.fetch_tests_from_worker = function(worker) {
  ...
  const remoteContext = this.create_remote_worker(worker);
 ...
 */
export function fetch_tests_from_worker(port) {
...
Tests.prototype.fetch_tests_from_window = function(remote) {
  ...
  const remoteContext = this.create_remote_window(remote);
  this.pending_remotes.push(remoteContext);
  return remoteContext.done;
};
 ...
 */
export function fetch_tests_from_window(window) {
 ...
 * @returns {Promise} - A promise that's resolved once all the remote tests are complete.
 */
export function fetch_tests_from_shadow_realm(realm) {
  const chan = new MessageChannel();
  function receiveMessage(msg_json) {
    chan.port1.postMessage(JSON.parse(msg_json));
  }
  const done = tests.fetch_tests_from_worker(chan.port2);
 ...
 * RemoteContext
 */
export function begin_shadow_realm_tests(postMessage) {
 ...
 * without arguments.
 */
export function add_start_callback(callback) {
 ...
 */
export function add_test_state_callback(callback) {
 ...
 * :js:class:`Test` as the only argument.
 */
export function add_result_callback(callback) {
 ...
 * empty.
 */
export function add_completion_callback(callback) {
...
Output.prototype.resolve_log = function() {
  ...
  const node = output_document.getElementById('log');
  if(!node) {
    ...
    if(output_document.body) {
      ...
      const root = output_document.documentElement;
      const is_html = root && root.namespaceURI == 'http://www.w3.org/1999/xhtml' && root.localName == 'html';
      const is_svg = output_document.defaultView && 'SVGSVGElement' in output_document.defaultView && root instanceof output_document.defaultView.SVGSVGElement;
      if(is_svg) {
        const foreignObject = output_document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject');
...
Output.prototype.show_status = function() {
  ...
  const done_count = tests.tests.length - tests.num_pending;
...
Output.prototype.show_results = function(tests, harness_status, asserts_run) {
  ...
  const log = this.output_node;
  if(!log) {
    return;
  }
  const output_document = this.output_document;

  while(log.lastChild) {
    log.removeChild(log.lastChild);
  }

  const stylesheet = output_document.createElementNS(xhtml_ns, 'style');
  stylesheet.textContent = stylesheetContent;
  const heads = output_document.getElementsByTagName('head');
  if(heads.length) {
  ...
  const summary_template = [
    ...
    ['h2', {}, 'Summary'],
    function() {
      const status = harness_status.format_status();
      const rv = [
        [
          'section',
          {},
    ...
    function() {
      ...
      while(Test.prototype.status_formats.hasOwnProperty(i)) {
        if(status_number.hasOwnProperty(Test.prototype.status_formats[i])) {
          const status = Test.prototype.status_formats[i];
          rv[0].push(['div', {}, ['label', {}, ['input', { type: 'checkbox', checked: 'checked' }], status_number[status] + ' ', ['span', { class: status_class(status) }, status]]]);
        }
        i++;
      }
      return rv;
  ...
  forEach(output_document.querySelectorAll('section#summary label'), function(element) {
    on_event(element, 'click', function(e) {
      ...
      const result_class = element.querySelector('span[class]').getAttribute('class');
      const style_element = output_document.querySelector('style#hide-' + result_class);
      const input_element = element.querySelector('input');
  ...
  const asserts_run_by_test = new Map();
  ...
  function get_asserts_output(test) {
    const asserts = asserts_run_by_test.get(test);
    if(!asserts) {
    ...
    rv += asserts
      .map(assert => {
        const output_fn = '<strong>' + escape_html(assert.assert_name) + '</strong>(';
        const prefix_len = output_fn.length;
        const output_args = assert.args;
        const output_len = output_args.reduce((prev, current) => prev + current, prefix_len);
        if(output_len[output_len.length - 1] > 50) {
          output_args = output_args.map((x, i) => (i > 0 ? '  '.repeat(prefix_len) : '') + x + (i < output_args.length - 1 ? ',\n' : ''));
        } else {
  ...
  const assertions = has_assertions();
  const html =
...
function substitute_single(template, substitutions) {
  ...
  function do_substitution(input) {
    ...
    const rv = [];
    for(const i = 0; i < components.length; i += 2) {
...
function make_dom_single(template, doc) {
  const output_document = doc || document;
  let element;
  if(template[0] === '{text}') {
    element = output_document.createTextNode('');
    for(const i = 1; i < template.length; i++) {
    ...
    for(const i = 2; i < template.length; i++) {
      if(template[i] instanceof Object) {
        const sub_element = make_dom(template[i]);
        element.appendChild(sub_element);
      } else {
        const text_node = output_document.createTextNode(template[i]);
...
function assert(expected_true, function_name, description, error, substitutions) {
  if(expected_true !== true) {
    const msg = make_message(function_name, description, error, substitutions);
...
const get_stack = function() {
  ...
  const re_text = script_url ? script_url.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') : '\\btestharness.js';
  const re = new RegExp(re_text + ':\\d+:\\d+');
...
export function OptionalFeatureUnsupportedError(message) {
...
function make_message(function_name, description, error, substitutions) {
  for(let p in substitutions) {
    if(substitutions.hasOwnProperty(p)) {
      substitutions[p] = format_value(substitutions[p]);
    }
  }
  const node_form = substitute(['{text}', '${function_name}: ${description}' + error], merge({ function_name: function_name, description: description ? description + ' ' : '' }, substitutions));
...
function expose(object, name) {
  ...
  const target = global_scope;
  for(const i = 0; i < components.length - 1; i++) {
...
function get_script_url() {
  ...
  for(const i = 0; i < scripts.length; i++) {
    ...
    const matches = src && src.match(/^(.*\/|)testharness\.js$/);
    if(matches) {
      return src;
    }
  }
...
if(global_scope.addEventListener) {
  const error_handler = function(error, message, stack) {
    const optional_unsupported = error instanceof OptionalFeatureUnsupportedError;
    if(tests.file_is_test) {
      const test = tests.tests[0];
      if(test.phase >= test.phases.HAS_RESULT) {
        return;
      }
      const status = optional_unsupported ? test.PRECONDITION_FAILED : test.FAIL;
      test.set_status(status, message, stack);
      test.phase = test.phases.HAS_RESULT;
    } else if(!tests.allow_uncaught_exception) {
      const status = optional_unsupported ? tests.status.PRECONDITION_FAILED : tests.status.ERROR;
