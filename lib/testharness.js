...
export function WindowTestEnvironment() {
  ...
  on_event(window, 'message', function(event) {
    if(event.data && event.data.type === 'getmessages' && event.source) {
      ...
      // this environment.
      for(let i = 0; i < this_obj.dispatched_messages.length; ++i) {
...
WindowTestEnvironment.prototype._dispatch = function(selector, callback_args, message_arg) {
  ...
  this._forEach_windows(function (w, same_origin) {
    if(same_origin) {
      try {
        let has_selector = selector in w;
      } catch(e) {
...
WorkerTestEnvironment.prototype._dispatch = function(message) {
  ...
  for(let i = 0; i < this.message_ports.length; ++i) {
...
function get_test_name(func, name) {
  ...
  if(func) {
    ...
    if(arrow !== null && !/[\u000A\u000D\u2028\u2029]/.test(func_code)) {
      let trimmed = (arrow[1] !== undefined ? arrow[1] : arrow[2]).trim();
...
export function test(func, name, properties) {
  ...
  if(value !== undefined) {
    let msg = 'Test named "' + test_name + '" passed a function to `test` that returned a value.';
...
export function async_test(func, name, properties) {
  ...
  if(func) {
    ...
    if(value !== undefined) {
      let msg = 'Test named "' + test_name + '" passed a function to `async_test` that returned a value (' + value + ').';
 ...
 * @param {Function} promiseOr - Either the constructor
 ...
 *
 */
export function promise_rejects_dom(test, type, promiseOr, descriptionOrPromise, maybeDescription) {
  let constructor, promise, description;
  if(typeof promiseOr === 'function' && promiseOr.name === 'DOMException') {
    constructor = promiseOr;
 ...
 *
 */
export function assert_throws_dom(type, funcOr, descriptionOrFunc, maybeDescription) {
  let constructor, func, description;
  if(funcOr.name === 'DOMException') {
    constructor = funcOr;
...
RemoteContext.prototype.test_state = function(data) {
  let remote_test = this.tests[data.test.index];
  if(!remote_test) {
...
export function Tests() {
  ...
  //console.log('Tests.prototype.set_timeout (1)', Tests.prototype.set_timeout);
  this.set_timeout();
}
...
//console.log('Tests.prototype.set_timeout (2)', Tests.prototype.set_timeout);

Tests.prototype.timeout = function() {
  let test_in_cleanup = null;
...
Output.prototype.resolve_log = function() {
  ...
  let node = output_document.getElementById('log');
  if(!node) {
...
Output.prototype.show_results = function(tests, harness_status, asserts_run) {
  ...
  forEach(output_document.querySelectorAll('section#summary label'), function(element) {
    on_event(element, 'click', function(e) {
      ...
      let style_element = output_document.querySelector('style#hide-' + result_class);
  ...
  function get_asserts_output(test) {
    ...
    rv += asserts
      .map(assert => {
        let output_fn = '<strong>' + escape_html(assert.assert_name) + '</strong>(';
        const prefix_len = output_fn.length;
        let output_args = assert.args;
...
function expose(object, name) {
  ...
  let target = global_scope;

  for(let i = 0; i < components.length - 1; i++) {
    if(!(components[i] in target)) target[components[i]] = {};
