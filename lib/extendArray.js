...
const { filter, findIndex, findLastIndex, indexOf, push, splice, unshift } = ArrayPrototype;
...
export const ArrayExtensions = {
  ...
  findLast(predicate) {
    ...
    if((i = findLastIndex.call(this, predicate)) == -1) return null;
  ...
  pushIf(item, pred = (arr, item) => arr.indexOf(item) == -1) {
    if(pred(this, item)) push.call(this, item);

    return this;
  },
  pushUnique(...args) {
    return args.reduce((acc, item) => (indexOf.call(this, item) == -1 ? (push.call(this, item), acc + 1) : acc), 0);
  },
  unshiftUnique(...args) {
    return args.reduce((acc, item) => (indexOf.call(this, item) == -1 ? (unshift.call(this, item), acc + 1) : acc), 0);
  },
  insert(item, sortFn = (a, b) => (a < b ? -1 : a > b ? 1 : 0)) {
    let index = findIndex.call(this, item2 => sortFn(item, item2) <= 0);
  ...
  delete(...items) {
    for(const item of items) {
      let index;
      if((index = indexOf.call(this, item)) != -1) splice.call(this, index, 1);
    }
    return this;
  },
  remove(...args) {
  ...
  removeIf(predicate) {
    ...
    for(let i = this.length - 1; i >= 0; --i) if(predicate(this[i], i, this)) splice.call(this, i, 1);

    return this;
  },
  rotateRight(n) {
    unshift.call(this, ...splice.call(this, n, this.length - n));
    return this;
  },
  rotateLeft(n) {
    push.call(this, ...splice.call(this, 0, n));
    return this;
  },
  ...
  match(callbackFn, thisArg) {
    ...
    return filter.call(this, callbackFn, thisArg);
  },
  group(callbackFn, thisArg) {
    let obj = {},
      i = 0;
    for(const item of this) (obj[callbackFn.call(thisArg, item, i++, this)] ??= []).push(item);
    return obj;
  },
...
export function extendArray(proto = ArrayPrototype) {
  const desc = Object.getOwnPropertyDescriptors(ArrayExtensions);
  ...
  for(const name in desc) {
    try {
      ...
      try {
        throw new TypeError(e.message + ` prop: ${name}`);
      } catch(e2) {}
    }
  }
