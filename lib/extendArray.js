const { weakDefine, filter, push, splice, unshift } = ArrayPrototype;
...
export const ArrayExtensions = {
  ...
  pushUnique(...args) {
    return args.reduce((acc, item) => (this.indexOf(item) == -1 ? (push.call(this, item), acc + 1) : acc), 0);
  },
  unshiftUnique(...args) {
    return args.reduce((acc, item) => (this.indexOf(item) == -1 ? (unshift.call(this, item), acc + 1) : acc), 0);
  },
  insert(item, sortFn = (a, b) => (a < b ? -1 : a > b ? 1 : 0)) {
    let index = this.findIndex(item2 => sortFn(item, item2) <= 0);
...
export const ArrayConstructor = ArrayPrototype.constructor;
