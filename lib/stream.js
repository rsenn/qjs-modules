...
function ReadableStreamBYOBReaderRead(reader, view, min, readIntoRequest) {
  ...
  if(STRM(stream).state == 'errored') readIntoRequest._errorSteps(STRM(stream).storedError);
    ...
    if(STRM(stream).readableStreamController.pendingPullIntos.length > 0) {
      STRM(stream).readableStreamController.pendingPullIntos.push(pullIntoDescriptor);
  ...
  function pullWithDefaultReader() {
    ...
    const readRequest = {
      ...
      _closeSteps: () => {
        ...
        if(STRM(branch1).readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(STRM(branch1).readableStreamController, 0);
        if(STRM(branch2).readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(STRM(branch2).readableStreamController, 0);
        if(!canceled1 || !canceled2) resolveCancelPromise(undefined);
      },
      _errorSteps: () => (reading = false),
    };
    ...
    const readIntoRequest = {
      ...
      _closeSteps: chunk => {
        ...
        if(chunk !== undefined) {
          ...
          if(!byobCanceled) ReadableByteStreamControllerRespondWithNewView(STRM(byobBranch).readableStreamController, chunk);
          if(!otherCanceled && STRM(otherBranch).readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(STRM(otherBranch).readableStreamController, 0);
        }
