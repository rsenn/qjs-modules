import { close, setReadHandler as onRead, setWriteHandler as onWrite } from 'os';
import { assert, define, fstat, isFunction, nonenumerable } from 'util';
import { Error as errorMap, getenv, strerror, sprintf, SEEK_SET, SEEK_CUR, SEEK_END } from 'std';
import { utf8Decode, toString, toArrayBuffer } from 'misc';
...
const DBG = /\bio\b/.test(getenv('DEBUG') ?? '')
  ...
  close(fd) {
    for(let m of [READ, WRITE]) {
      ...
      if((e = this.#fds.get([fd, m])))
        throw new Error(`Cannot close fd ${fd}, because there is still a ${m ? 'write' : 'read'} handler`);
    }
    close(fd);
  }
}
...
export const IOReadDecorator = nonenumerable({
  ...
  getline() {
    ...
    while((res = this.read(u8.buffer, 0, 1)) > 0) {
      let n = res;
      while((res = utf8Decode(u8.buffer, 0, n)) === null) if((res = this.read(u8.buffer, n, 1)) > 0) n += res;
      const [code, len] = res;
      if(code == 10) break;
