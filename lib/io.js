...
import { assert, define, isFunction, nonenumerable } from 'util';
import { Error as errorMap, getenv, strerror, sprintf, SEEK_SET, SEEK_CUR, SEEK_END } from 'std';
import { fstat, utf8Decode, toString, toArrayBuffer } from 'misc';
...
const DBG = /\bio\b/.test(getenv('DEBUG') ?? '') ? (...args) => console.log('\x1b[1;33mIO\x1b[0m', ...args.map(s => s.replace(/\s+/g, ' '))) : () => {};
