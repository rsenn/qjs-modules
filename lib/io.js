...
import { fstat, toArrayBuffer, toString } from 'misc';
import { Error as errorMap, getenv, SEEK_CUR, sprintf, strerror } from 'std';
...
const DEBUG = /\bio\b/.test(getenv('DEBUG') ?? '') ? (...args) => console.debug('\x1b[1;33mIO\x1b[0m', ...args.map(s => s.replace(/\s+/g, ' '))) : () => {};
