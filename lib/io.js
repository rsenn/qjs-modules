...
const DBG = getenv('DEBUG') ? (...args) => console.log('\x1b[1;33mIO\x1b[0m', ...args) : () => {};
  ...
  check(mode) {
    for(let fd of this.fds(mode))
      if(!checkFileDescriptor(fd))
        this.#fds[fd].forEach((fd, i) => {
          console.log(`Got a ${i ? 'writ' : 'read'}able handler for invalid file descriptor ${fd}`);
          this.#fds.delete([fd, i]);
        });
  }
...
export function setReadHandler(fd, callback) {
  DBG(`setReadHandler(${fd}, ${(callback + '').replace(/\s+/g, ' ')})`);
  return io.on([fd, READ], callback);
}

export function setWriteHandler(fd, callback) {
  DBG(`setWriteHandler(${fd}, ${(callback + '').replace(/\s+/g, ' ')})`);
