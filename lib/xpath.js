...
export function buildXPath(ptr, root) {
  ...
  for(let key of ptr) {
    ...
    if(parent && 'children' in parent) {
      ...
      siblingSame = siblings.reduce((acc, n, i) => {
        if(isObject(n) && isObject(node[key]))
          if(n.tagName == node[key].tagName) acc.push([i, n.tagName]);
        return acc;
      }, []);
    } else {
      siblings = [];
    ...
    if(!isObject(node))
      throw new Error(
        `Error indexing <${typeof node}> with <${key}> [${hier.join(', ')}]: ${parts
          .map(GetKey)
          .join(' -> ')}`
      );
...
function GetKey(p) {
  ...
  if((isObject(p) || isFunction(p)) && p instanceof Predicate) {
    ...
    typeof p == 'object' &&
    ['tagName', 'children', 'attributes'].some(prop => prop in p)
  ) {
    k = p.tagName;
    ...
    throw new Error(
      `Invalid part (${util.getTypeStr(p)} ${typeof p}): ${inspect(p, { colors: true })}`
    );
  return k ?? p;
}
