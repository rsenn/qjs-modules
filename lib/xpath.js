...
export class XPath extends Array {
  ...
  [Symbol.inspect](depth, opts = {}) {
    ...
    //console.log('obj', obj);
    return `\x1b[1;31m${this.constructor.name ?? util.getClassName(this)}\x1b[0m ` + inspect(obj ?? { __proto__: { [Symbol.toStringTag]: 'XPath' }, ...obj }, { ...opts, customInspect: true });
  }
...
export function buildXPath(ptr, root) {
  ...
  for(let key of ptr) {
    ...
    if(type != 'array') {
      ...
      siblingSame = siblings.reduce((acc, n, i) => {
        if(isObject(n) & isObject(node[key])) if (n.tagName == node[key].tagName) /*if (i != key)*/ acc.push([i, n.tagName]);
        return acc;
      }, []);
    }
  ...
  function NodeType(node) {
    return isObject(node) ? ('length' in node && typeof node.length == 'number' ? 'array' : util.getClassName(node).toLowerCase()) : typeof node;
  }
