...
import { /*getcwd,*/ chdir, SIGTERM } from 'os';
import { getCommandLine, getCurrentWorkingDirectory, getExecutable, getFileDescriptor, hrtime, uname, kill, getpid /*getuid, getgid, geteuid, getegid, setuid, setgid, seteuid, setegid*/ } from 'misc';
import { isRelative, join, getcwd, absolute, relative, resolve, normalize } from 'path';
...
const process = {
  ...
  async importModule(p) {
    ...
    g
      ...
      : (globalThis[p.slice(p.lastIndexOf('/') + 1).replace(/\.[^\/.]+$/g, '')] = m);
    return m;
  }
};
