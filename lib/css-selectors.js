...
function AttributeSelector(node) {
  if(!node.operator) return Predicate.property('attributes', Predicate.has(node.name));
  ...
  return Predicate.property('attributes', Predicate.property(node.name, Predicate.regexp(source, flags)));
}
