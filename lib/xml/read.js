...
function parse2(g) {
  ...
  if(typeof g[Symbol.iterator] == 'function') g = g[Symbol.iterator]();
  ...
  const str = typeof s == 'string' ? s => s : bufferToString;
  const skip = pred => {
    data = [];
    while(!done) {
      if(m[c] & BACKSLASH) {
...
export const parse = process.release.name == 'quickjs' ? requireModule('xml').read : xmlParse;
