/* Named entity table + character-reference decode/encode, ported from
 * include/xml_entities.h (xml_decode_entities/xml_lookup_named_entity), shared by
 * every native XML reader in this codebase. Kept here as a single source of truth
 * so lib/xml/read.js and lib/xml/write.js (and anything else parsing/serializing
 * XML in JS) stay in sync with the C implementation's entity handling. */
 ...
 * dbuf_put_escaped_xml() - attribute values here are always double-quoted). */
export function escapeXml(str, isAttr) {
  let out = '';
...
export default { decodeEntities, escapeXml };
