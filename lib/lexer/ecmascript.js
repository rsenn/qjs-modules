...
export { Location, Token } from 'lexer';

export const ECMAScriptDefines = {
  DecimalDigit: /[0-9]/,
  ...
  IdentifierStart: /[$_a-zA-Z]|{UnicodeIdentifierStart}|"\\"[u]{HexDigit}{4}/,
  IdentifierPart: /{IdentifierStart}|{UnicodeIdentifierPart}|[0-9]/,
  Identifier: /({IdentifierStart})({IdentifierPart})*/,
  ExponentIndicator: /[eE]/,
  SignedInteger: /[+-]?[0-9]+/,
  DecimalIntegerLiteral: /([0]|{NonZeroDigit}(_?{DecimalDigit})*)/,
...
export const ECMAScriptRules = [
  ...
  ['comment', '<INITIAL,NOREGEX,NESTED>{SingleLineComment}|{MultiLineComment}'],

  ['regexpLiteral', '<INITIAL>{RegularExpressionLiteral}'],

  ['templateLiteral', /<INITIAL,NOREGEX,NESTED>`{TemplateElement}*`/],
  [
    'templateLiteralHead',
    /<INITIAL,NOREGEX,NESTED>`{TemplateElement}*\${/,
    (lex, skip) => {
  ...
  [
    ...
    /<TEMPLATE>{TemplateElement}*`/,
    lex => {
      //util.assertEqual(lex.topState(), 'TEMPLATE')
      try {
        lex.popState();
      } catch(e) {}
    },
  ...
  [
    'punctuator',
    //  /<INITIAL,NOREGEX,NESTED>(-->>=|>>>=|\?\?=|&&=|\|\|=|\*\*=|\.\.\.|<<=|-->>|>>=|>>>|===|!==|\?\?|\*\*|\?\.|$\{|=>|%=|-=|>=|\^=|\+=|--|<=|\|=|\+\+|==|&=|>>|\|\||\/=|<<|&&|\*=|!=|@|\{|\^|\+|:|\)|\||\?|;|\(|&|~|\]|\/|!|<|\[|\*|,|>|\}|%|-|\.|=)/
    '<INITIAL,NOREGEX,NESTED>(-->>=|>>>=|\\?\\?=|&&=|\\|\\|=|\\*\\*=|\\.\\.\\.|<<=|-->>|>>=|>>>|===|!==|\\?\\?|\\*\\*|\\?\\.|$\\{|=>|%=|-=|>=|\\^=|\\+=|--|<=|\\|=|\\+\\+|==|&=|>>|\\|\\||\\/=|<<|&&|\\*=|!=|@|\\{|\\^|\\+|:|\\)|\\||\\?|;|\\(|&|~|\\]|\\/|!|<|\\[|\\*|,|>|\\}|%|-|\\.|=)',
  ],
  ['numericLiteral', '<INITIAL,NOREGEX,NESTED>({OctalIntegerLiteral}|{HexIntegerLiteral}|{BinaryIntegerLiteral}|{DecimalLiteral})[lmn]?'],
...
export class ECMAScriptLexer extends Lexer {
  constructor(input, fileName) {
    ...
    //console.log('ECMAScriptLexer.constructor', { fileName });
  ...
  addDefines() {
    for(let name in ECMAScriptDefines) this.define(name, ECMAScriptDefines[name]);
  }

  addRules() {
    for(let rule of ECMAScriptRules) this.addRule(...rule);
  }
}

globalThis.ECMAScriptLexer = ECMAScriptLexer;

export { ECMAScriptLexer as Lexer };
