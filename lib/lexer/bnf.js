...
import { ECMAScriptDefines, ECMAScriptRules } from './ecmascript.js';
...
export class BNFLexer extends Lexer {
  ...
  constructor(input, filename) {
    ...
    this.handler = (arg, tok) => console.log('Unmatched token at ' + arg.loc + ' state=' + arg.states[arg.state] + '\n' + arg.currentLine() + '\n' + ' '.repeat(arg.loc.column - 1) + '^');
  ...
  addRules() {
    ...
    this.addRule('rbrace', /}/);
    this.addRule('section', /%%/, (tok, lexeme, state) => (++this.section == BNFLexer.section.SCANNERCODE ? (this.beginCode ? this.beginCode(tok, lexeme, state) : this.pushState(globalThis.code ?? 'C')) : undefined));
    ...
    for(let [name, expr, mask = JS] of ECMAScriptRules) {
      let fn;
