...
import { isArray, isArrayBuffer, isBigDecimal, isBigFloat, isBigInt, isBool, isConstructor, isEmptyString, isError, isException, isExtensible, isFunction, isInstanceOf, isInteger, isJobPending, isLiveObject, isNull, isNumber, isObject, isRegisteredClass, isString, isSymbol, isUncatchableError, isUndefined, isUninitialized, } from 'misc';
...
//Object.assign(globalThis, { JSValue, JSNumber, JSString, JSBoolean, JSSymbol, JSRegExp, JSObject, JSArray, JSTypedArray, JSFunction, JSConstructor, JSProperty });
...
export function EncodeJS(val) {
  ...
  switch (type) {
    ...
    case 'object':
      ...
      const is_array = type == 'object' && Array.isArray(val) && val !== Array.prototype;
      const is_typedarray = (Object.getPrototypeOf(val) == TypedArrayPrototype || val instanceof TypedArrayPrototype.constructor) && val !== TypedArrayPrototype;
      const is_arraybuffer = (Object.getPrototypeOf(val) == ArrayBuffer.prototype || val instanceof ArrayBuffer) && val !== ArrayBuffer.prototype;
      
      if(is_function) {
        info = new (is_constructor ? JSConstructor : JSFunction)(/=>/.test(val + '') ? (val + '').replace(/^\((.*)\)\s*=>\s/, '($1) => ') : (val + '').replace(/^(function\s*|)(.*)/, '(function $2)'));
      } else if(val instanceof RegExp) {
        info = new JSRegExp(val.source, val.flags);
      } else if(is_array || is_typedarray) {
        info = new (is_typedarray ? JSTypedArray : JSArray)(is_typedarray ? null : [...val].map(v => EncodeJS(v)));
      } else {
        info = new JSObject();
      }
      ...
      if(!is_function) {
        ...
        const members = !is_object ? [...val].map(i => EncodeJS(i)) : EncodeObj(val);

        if(is_typedarray) (info.members ??= {}).buffer = EncodeJS(val.buffer);
        else if(!is_arraybuffer && is_object && Object.keys(members).length > 0) info.members = members;

        if(!is_arraybuffer && is_object) define(info, { proto: EncodeJS(Object.getPrototypeOf(val)) });
      }
      break;
    case 'number':
    case 'bigfloat':
