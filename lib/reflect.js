...
import { getPrototypeChain, isConstructor, isArrowFunction, mapFunction } from 'util';
...
/* prettier-ignore */ define(JSNumber.prototype, { [Symbol.toStringTag]: 'JSNumber', type: 'number', [Symbol.inspect](depth, opts = {}) { return inspect(this, opts).replace(/\s*{.*/gs, ' ' + inspect(this.value, opts)); } });
/* prettier-ignore */ define(JSString.prototype, { [Symbol.toStringTag]: 'JSString', type: 'string', [Symbol.inspect](depth, opts = {}) { return inspect(this, opts).replace(/\s*{.*/gs, ` \x1b[36;1m${inspect(this.value, { ...opts, colors: false })}\x1b[0m`); } });
...
const C = (depth = 2, opt = {}) => console.config({ depth, compact: depth - 1, ...opt });
...
function cleanFunctionCode(s) {
  if(s) return (s + '').replace(/^(function\s*|)(get\s*|)(set\s*|)(.*)/gs, 'function $4');
}
  ...
  switch (type) {
    ...
    case 'object':
      ...
      if(is_function) {
        ...
        info = new (is_constructor ? JSConstructor : JSFunction)(code);
      ...
      if(!is_function || is_constructor || is_object) {
        ...
        if(is_constructor) console.log('EncodeJS', { keys });

        if(!('members' in info))
          if(keys.length > 0) {
            Object.assign(info, {
              members: EncodeObj(val, (key, desc) => true, [...stack, val], mapFn),
            });
          }
      }
