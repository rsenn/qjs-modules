...
/* prettier-ignore */ class JS extends JSFunction { constructor(code, members, proto) { super(code, members, proto); } }
...
define(JSValue.prototype, {
  ...
  toJSON() {
    ...
    switch (this.type) {
      ...
      case 'array': {
        ret.members = [];
        for(const elem of this.members) ret.members.push(elem?.toJSON() ?? elem);
        break;
      }
      case 'object':
        if(this.value === null) {
...
define(JS.prototype, { [Symbol.toStringTag]: 'JS', type: 'constructor' });
...
export { JSValue, JSNumber, JSString, JSBoolean, JSSymbol, JSRegExp, JSObject, JSArray, JSTypedArray, JSFunction, JS, JSProperty };

const C = globalThis?.console?.config ? (depth = 2, opt = {}) => console.config({ depth, compact: depth - 1, ...opt }) : () => '';

export const TypedArrayPrototype = Object.getPrototypeOf(Uint32Array.prototype);
export const TypedArray = TypedArrayPrototype.constructor;
...
export function* getKeys(obj, t = (desc, key) => true) {
  ...
  for(const key in a) if(t(a[key], key, a)) yield key;
}
...
export function EncodeJS(val, stack = [], mapFn = () => {}) {
  ...
  switch (type) {
    ...
    case 'object':
      ...
      const is_constructor = is(val) || (is_function && 'prototype' in val);
      ...
      if(is_function) {
        ...
        info = new (is_constructor ? JS : JSFunction)();
        define(info, { code });
...
export function DecodeJS(info) {
  ...
  switch (info.type) {
    ...
    case 'array': {
      val = [];
      for(const item of info.members) val.push(DecodeJS(item));
      break;
    }
    case 'constructor':
    case 'function':
...
function define(obj, ...args) {
  for(const other of args) {
    ...
    for(const k of keys) props[k] = { enumerable: false, configurable: true, writable: true, value: desc[k].value };
