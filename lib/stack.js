...
Stack.prototype[inspectSymbol] = function (depth, options) {
  return `\x1b[1;31m${this[Symbol.toStringTag]}\x1b[0m ` + globalThis.inspect([...this], depth, { ...options, breakLength: 1000, compact: 1 });
};
...
export class StackFrame {
  ...
  get loc() {
    ...
    let obj = {};
    for (let prop of ['fileName', 'lineNumber', 'columnNumber']) if (prop in this) obj[prop] = !isNaN(+this[prop]) ? +this[prop] : this[prop];

    return {
      ...obj,
      toString() {
        let a = [];
        for (let prop of ['fileName', 'lineNumber', 'columnNumber']) if (prop in this) a.push(this[prop]);
        return a.join(':');
      }
    };
  }
