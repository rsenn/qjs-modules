...
/*export function Location(loc) {
  let obj = new.target ? this : new Location(loc);
  if(typeof loc == 'object' && loc != null) {
    for(let prop of ['fileName', 'lineNumber', 'columnNumber'])
      if(prop in loc) obj[prop] = loc[prop];
  }
  return obj;
}

Location[Symbol.species] = Location;

Location.prototype[Symbol.species] = Location;
Location.prototype[Symbol.toStringTag] = 'Location';
Location.prototype[inspectSymbol] = function(depth, options) {
...
Object.assign(Location.prototype, {
  toString() {
    let a = [];
    for(let prop of ['fileName', 'lineNumber', 'columnNumber'])
      if(prop in this) a.push(this[prop]);
    return a.join(':');
  }
});
Object.assign(Location, {
  fromString(str) {
    let parts = str.split(/:/g).map(p => (!isNaN(+p) ? +p : p));
    if(parts.length == 0 || typeof parts[0] == 'number') parts.unshift(null);
    let obj = { fileName: parts[0] };

    if(parts[1]) obj.lineNumber = parts[1];
    if(parts[2]) obj.columnNumber = parts[2];
...
export function Stack(st, pred = fr => true) {
  ...
  return Object.setPrototypeOf({ ...st, length: st.length }, Stack.prototype);
}

Stack[Symbol.species] = Stack;

Stack.prototype[Symbol.species] = Stack;
...
Object.assign(Stack, {
  ...
  common(a, b) {
    ...
    let i;
    for(i = 0; i < len; i++)
      if(!StackFrame.equal(a[a.length - (i + 1)], b[b.length - (i + 1)])) break;
    return i;
  },
  update(stack, newStack = new Stack()) {
...
export class StackFrame {
  ...
  get loc() {
 /*    if(!('fileName' in this) && !('lineNumber' in this)) return null;
   let obj = {};
    for(let prop of ['fileName', 'lineNumber', 'columnNumber']) if(prop in this) obj[prop] = {value: !isNaN(+this[prop]) ? +this[prop] : this[prop], enumerable: true };
*/
