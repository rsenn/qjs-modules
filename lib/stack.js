...
const AddMethods = (props, MakeSetter) => {
  for(let i = 0; i < props.length; i++) {
    StackFrame.prototype['get' + Capitalize(props[i])] = MakeGetter(props[i]);
    StackFrame.prototype['set' + Capitalize(props[i])] = MakeSetter(props[i]);
  }
};

// prettier-ignore
AddMethods(booleanProps, p => function(v) { this[p] = Boolean(v); });
// prettier-ignore
AddMethods(numericProps, p => function(v) {
      if(!isNumber(v)) throw new TypeError(p + ' must be a Number');
      this[p] = Number(v);
    });
// prettier-ignore
AddMethods(stringProps, p => function(v) { this[p] = String(v); });
