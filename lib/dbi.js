...
export class Result {
  ...
  /* fetchAssoc() is preferred over the raw driver's own Symbol.iterator/
   * Symbol.asyncIterator: the plain iterator protocol's returned iterator
   * is a distinct object from the result it came from, so a `resultType`
   * flag set on the result itself (the usual way to ask for row objects
   * instead of positional arrays) has no effect on it - fetchAssoc()
   * always returns row objects regardless, uniformly across all three
   * drivers, sidestepping that mismatch entirely. */
  async *[Symbol.asyncIterator]() {
    const r = this._raw;
    if(!r || typeof r !== 'object') return;

    if(typeof r.fetchAssoc === 'function') {
      let row;
      while((row = await r.fetchAssoc()) != null) yield row;
      return;
    }
...
class PGSQLAdapter {
  async connect(options) {
    ...
    else await this.db.connect(options.host ?? 'localhost', options.user ?? '', options.password ?? '', options.database ?? options.dbname ?? '', options.port ?? 5432, options.timeout ?? 10);
  }
...
class MySQLAdapter {
  async connect(options) {
    ...
    await this.db.connect(options.host ?? 'localhost', options.user ?? '', options.password ?? '', options.database ?? options.dbname ?? '');
  }
