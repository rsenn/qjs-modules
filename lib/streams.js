...
import { define, error, F_GETFL, F_SETFL, fcntl, O_NONBLOCK, quote, toArrayBuffer, toString } from 'util';
...
ReadableStream.prototype.values = ReadableStream.prototype[Symbol.asyncIterator] = async function* () {
  const reader = this.getReader();
  let item;

  while((item = await reader.read())) {
...
export function FileSystemReadableStream(file, bufSize = 1024 * 64) {
  ...
  return new ReadableStream({
    ...
    pull(controller) {
      ...
      try {
        ret = fs.readSync(file, buf, 0, bufSize);
      } catch(e) {
        err.exception = e;
      }

      if(ret > 0) controller.enqueue(ret == bufSize ? buf : buf.slice(0, ret));
      else if(err.exception) controller.error(file);
...
export async function* StreamReadIterator(strm) {
  ...
  //  for await(const chunk of await reader.read()) yield chunk;
...
export async function* LineStreamIterator(strm) {
  ...
  for await(const chunk of await strm) {
