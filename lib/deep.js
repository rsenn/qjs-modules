...
export function forEach(...args) {
  ...
  if(Util.isObject(value)) for(const k in value) forEach(value[k], fn, path.concat([isNaN(+k) ? k : +k]), root);
}
  ...
  gen = function* (...args) {
    ...
    if(r !== -1)
      if(typeof value == 'object' && value != null) {
        for(const k in value) yield* gen(value[k], filter, flags, path.concat([isNaN(+k) ? k : +k]), root);
      }
  };
...
export function unflatten(map, obj = {}) {
  for(const [path, value] of map) set(obj, path, value);
