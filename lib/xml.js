...
export function parse(s) {
  ...
  while(i < n) {
    ...
    if(m[s[i]] & START) {
      ...
      if(name[0] == '!' || ['br', 'hr', 'img'].indexOf(name) != -1) self_closing = true;
      ...
      if(self_closing) {
        if(e?.children?.length === 0) delete e.children;

        //if(name == 'br') console.log('st[0]', { e });
...
export function write(obj, level = 0) {
  ...
  if('attributes' in obj) if (typeof obj.attributes == 'object') for(const attr in obj.attributes) s += ` ${attr}` + (obj.attributes[attr] === true ? '' : `="${obj.attributes[attr]}"`);
