...
export function readdirSync(path, { withFileTypes, recursive, encoding } = {}) {
  if(recursive) {
    ...
    for(const entry of walkSync(path, { includeDirs: true, includeFiles: true, includeSymlinks: true })) {
      ...
      out.push(withFileTypes ? Object.assign(new Dirent(entry.name, entry.isDirectory ? Directory.TYPE_DIR : entry.isSymlink ? Directory.TYPE_LNK : Directory.TYPE_REG), { path: entry.path, parentPath: dirname(entry.path) }) : rel);
    }
