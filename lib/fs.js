...
export function puts(fd, str) {
  if(typeof fd == 'object' && fd && typeof fd.puts == 'function') {
    ...
    return write(fd, data, 0, data.byteLength);
  }
}
...
export function sizeSync(file) {
  let bytes, pos;
  let fd = typeof file == 'number' ? file : open(file, 'r');
  pos = tell(fd);
...
export function readAll(input, bufSize = 1024) {
  const buffer = buffer(bufSize);
  let output = '';
  let ret;
  do {
    //await waitRead(input);
    ret = read(input, buffer, 0, bufSize);
    //console.log('readAll', { ret, input: fileno(input), buffer });
    let str = bufferToString(buffer.slice(0, ret));
    output += str;
