...
export function readdirSync(path, { withFileTypes, encoding } = {}) {
  ...
  for(const [name, type] of new Directory(path)) {
    ...
    result.push({
      name,
      parentPath: path,
      isBlockDevice: () => !!(type & Directory.TYPE_BLK),
      isCharacterDevice: () => !!(type & Directory.TYPE_CHR),
      isDirectory: () => !!(type & Directory.TYPE_DIR),
      isFIFO: () => !!(type & Directory.TYPE_FIFO),
      isFile: () => !!(type & Directory.TYPE_REG),
      isSocket: () => !!(type & Directory.TYPE_SOCK),
      isSymbolicLink: () => !!(type & Directory.TYPE_LNK),
    });
  }
...
export async function readFully(input, buf, start = 0, n) {
  ...
  while(start < n) {
    const r = await read(input, buf, start, n - start);
    if(r <= 0) return new Error(`input ${r < 0 ? 'error' : 'closed'} ${start} of ${n} bytes read`);
    start += r;
  }
...
export function watch(filename, options = {}, callback = (eventType, filename) => {}) {
  ...
  options.mask ??= IN_ALL_EVENTS; // IN_MODIFY | IN_MOVE_SELF | IN_MOVED_FROM | IN_MOVED_TO | IN_DELETE | IN_DELETE_SELF | IN_CREATE | IN_CLOSE_WRITE | IN_ATTRIB;

  let fd, wd;

  try {
    fd = iwatch();
    wd = iwatch(fd, filename, options.mask);
  } catch(err) {
    return err;
  }
  ...
  const buf = new ArrayBuffer(16 + 1024 + 1);

  os.setReadHandler(fd, () => {
    let r = os.read(fd, buf, 0, buf.byteLength);

    for(let i = 0; i < r; ) {
      const arr = new Uint32Array(buf, i, 4);
      let [wd, mask, cookie, len] = arr;

      let filename, name;

      mask &= ~(IN_CLOSE_NOWRITE | IN_ISDIR);

      console.log('watch', console.config({ compact: true }), { mask, cookie, len });

      if(len > 0) {
        filename = buf.slice(i + 16, i + 16 + len);
        let /*end = new Uint8Array(filename).indexOf(0);
        if(end == -1)*/ end = len;
        name = toString(filename, 0, end);
      }

      i += 16 + len;

      ret.emit(mask & (IN_CREATE | IN_MOVED_TO | IN_MOVED_FROM | IN_DELETE | IN_DELETE_SELF) ? 'rename' : 'change', name);
    }
  });
