...
import { F_OK, R_OK, W_OK, X_OK, fchmod, fchown, fsync, error, futimes, fstat } from 'misc';
...
function savePos(file, fn, position) {
  const savedPos = file.tell();
  if(typeof position == 'number') file.seek(position, std.SEEK_SET);
  const ret = fn();
  if(typeof position == 'number') file.seek(savedPos, std.SEEK_SET);
  return ret;
}
...
/**
 * https://nodejs.org/dist/latest/docs/api/fs.html#fs_class_filehandle
 *
 * @class      FileHandle
 */
export class FileHandle {
  ...
  get fd() {
    return fileno(this.#f);
  }

  async appendFile(data, options = { encoding: 'utf8' }) {
    const encoding = typeof options == 'string' ? options : options.encoding;

    if(typeof data == 'string') this.#f.puts(data);
    else this.#f.write(data, 0, data.byteLength);
  }
  ...
  async read(buffer, offset, length, position) {
    return { bytesRead: savePos(this.#f, () => this.#f.read(buffer, offset, length), position), buffer };
  }

  async readFile(options = { encoding: null, flag: 'r' }) {
    const encoding = typeof options == 'string' ? options : options.encoding;

    if(encoding == 'utf8') return this.#f.readAsString();

    const filePos = this.#f.tell();
    this.#f.seek(0, std.SEEK_END);
    const size = this.#f.tell();
    this.#f.seek(filePos, std.SEEK_SET);

    const buffer = new ArrayBuffer(size);
    const bytesRead = this.#f.read(buffer, 0, size);

    if(bytesRead == size) return buffer;
  }

  async stat(options = { bigint: false }) {
  ...
  async write(buffer, offset, length, position) {
    if(typeof buffer == 'string') {
      ...
      return {
        bytesWritten: savePos(
          this.#f,
          tellDiff(this.#f, () => this.#f.puts(buffer)),
          position
        ),
        buffer
      };
    ...
    return { bytesWritten: savePos(this.#f, () => this.#f.write(buffer, offset, length), position), buffer };
  }

  async writeFile(data, options = { encoding: 'utf8' }) {
    const encoding = typeof options == 'string' ? options : options.encoding;
    if(typeof data == 'string') this.#f.puts(data);
    else this.#f.write(data, 0, data.byteLength);
  }
...
export function openSync(filename, flags = 'r', mode = 0o644) {
  ...
  return fopen(filename, flags, mode);
