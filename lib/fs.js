...
import { toString as ArrayBufferToString, toArrayBuffer as StringToArrayBuffer, access, fcntl, error } from 'misc';
...
export function fcntlSync(fd, cmd, ...args) {
...
export function writeFileSync(file, data, overwrite = true) {
  ...
  if(typeof data == 'string') {
    ...
    let fd = os.open(file, O_WRONLY | O_CREAT | (overwrite ? O_TRUNC : O_EXCL), 0o644);
    if(fd >= 0) {
