...
export function createReadStream(path, options = { encoding: 'utf8' }) {
  const { encoding, bufSize = 1024, start = 0, end = Infinity } = typeof options == 'string' ? { encoding: options } : options;
  let file,
    err = {},
    buf,
    decoder,
...
export function createWriteStream(path, options = { encoding: 'utf8' }) {
  const { encoding, start = 0 } = typeof options == 'string' ? { encoding: options } : options;
  let file,
    err = {},
    bytesWritten = 0,
    encoder;
