...
import { IN_ATTRIB, IN_CLOSE_WRITE, IN_CREATE, IN_DELETE, IN_DELETE_SELF, IN_MODIFY, IN_MOVED_TO, IN_MOVE_SELF, access, error, fchmod, fchown, fdatasync, fstat, fsync, ftruncate, futimes, isNumber, isString, isObject, isArrayBuffer, mkstemp, tempnam, toArrayBuffer, toString, watch, link as sys_link, symlink as sys_symlink, } from 'misc';
...
function syscallerr(name, ret) {
  if(typeof ret == 'number') {
    if(ret < 0) throw new SyscallError(name, -ret);

    return ret || 0;
  } else if(Array.isArray(ret)) {
    const [str, err] = ret;

    if(err) throw new SyscallError(name, err);

    return str;
  }

  throw new SyscallError(name/*, SyscallError.errno()*/);
}
...
export async function link(existingPath, newPath) {
  if(sys_link(existingPath, newPath) == -1) throw new SyscallError(`link`, error().errno);
}
