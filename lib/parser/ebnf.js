...
import Grammar, { Alternatives as GAlternatives, Empty as GEmpty, NonTerminal as GNonTerminal, OneOrMore as GOneOrMore, Optional as GOptional, ParseNode, Sequence as GSequence, Terminal as GTerminal, ZeroOrMore as GZeroOrMore, alt, empty, many, opt, ref, seq, some, t, } from './grammar.js';
...
function Node(token) {
  ...
  if(token) {
    ...
    const ctor = { identifier: Terminal, r_identifier: Terminal, char_class: Terminal, literal: Terminal, ws: Terminal }[type];
...
export class EBNFParser extends Parser {
  ...
  parseProduction() {
    ...
    for(const [tokens, action] of results) {
      console.log('id.lexeme',id.lexeme);
...
/**
 * buildGrammar(source, filename)
 *
 * Reads a BNF/yacc-style grammar and returns a `Grammar` object composed
 * from the classes in `./grammar.js`. Supports:
 *   - `%token NAME [NAME2 ...]` declarations (marks a symbol as terminal)
 *   - `%start NAME`
 *   - production rules `name : alt1 | alt2 | ... ;`
 *   - alternatives, sequences, `?`, `*`, `+`
 *   - single-quoted literals like `'if'`
 *   - `/* ... *\/` C-style comments and `//` line comments (handled by BNFLexer)
 *
 * The returned Grammar has terminals matched against `token.type` (for
 * declared %token names) or `token.lexeme` (for quoted literals or bare
 * identifiers that were never declared).
 */
export function buildGrammar(source, filename) {
  const lex = new BNFLexer(source, filename ?? 'grammar.y');

  /* pull all tokens ahead of time, skipping whitespace/comments the BNF
   * lexer surfaces but that carry no grammatical weight */
  const skip = new Set(['x_ws', 'ws', 'l_ws', 'd_ws', 'p_ws', 'r_ws', 'multiline_comment', 'singleline_comment', 'c_ws', 'c_newline']);
  const tokens = [];
  for(const tok of lex) if(!skip.has(tok.type)) tokens.push(tok);

  let pos = 0;
  const peek = (o = 0) => tokens[pos + o];
  const eat = type => {
    const tok = tokens[pos];
    if(!tok) throw new Error(`Unexpected EOF, wanted ${type}`);
    if(type && tok.type !== type && tok.lexeme !== type) throw new Error(`${tok.loc}: expected ${type}, got ${tok.type} '${tok.lexeme}'`);
    pos++;
    return tok;
  };

  const declaredTokens = new Set();
  const declaredLiterals = new Map(); /* literal text -> token type (unknown) */
  let startName = null;
  const productions = new Map(); /* name -> array of alternative Sequences */

  /* ---- prologue: directives until %% ---- */
  while(pos < tokens.length && peek().type !== 'section2') {
    const tok = peek();
    if(tok.type === 'directive') {
      pos++;
      const name = tok.lexeme.slice(1); /* strip '%' */
      const args = [];
      while(pos < tokens.length) {
        ...
        if(p.type === 'directive' || p.type === 'section2') break;
        args.push(p);
        pos++;
      }
      if(name === 'token') {
        for(const a of args) if(a.type === 'd_identifier' || a.type === 'identifier') declaredTokens.add(a.lexeme);
      } else if(name === 'start' && args.length) {
        startName = args[0].lexeme;
      }
      /* other directives (%left, %right, %type, ...) are ignored */
    } else {
      pos++; /* skip stray token in prologue */
    }
  }

  /* ---- consume %% ---- */
  if(pos < tokens.length && peek().type === 'section2') pos++;

  /* ---- productions ---- */
  while(pos < tokens.length) {
    const tok = peek();
    if(!tok || tok.type === 'section2') break;

    /* skip blank tokens */
    if(tok.type === 'p_newline' || tok.type === 'l_newline' || tok.type === 'd_newline') {
      pos++;
      continue;
    }

    if(tok.type !== 'identifier' && tok.type !== 'r_identifier') {
    ...
    /* expect ':' */
    while(pos < tokens.length && (peek().type === 'p_newline' || peek().type === 'd_newline')) pos++;
    if(pos >= tokens.length) break;
    const colonTok = peek();
    if(!(colonTok && (colonTok.type === 'colon' || colonTok.type === 'r_colon' || colonTok.lexeme === ':')))
      throw new Error(`${colonTok?.loc}: expected ':' after '${name}', got '${colonTok?.lexeme}'`);
    pos++;
    ...
    while(pos < tokens.length) {
      const p = peek();
      if(!p) break;
      if(p.type === 'bar' || p.type === 'r_pipe' || p.lexeme === '|') {
        pos++;
        alternatives.push(parseAlternative());
      } else if(p.type === 'semi' || p.type === 'r_semi' || p.lexeme === ';') {
        pos++;
        break;
      } else {
        break;
    ...
    productions.get(name).push(...alternatives);
    if(startName == null) startName = name;
  }

  /* ---- promote undefined NonTerminals to Terminals ---- */
  /* If an identifier is referenced but never appears on the LHS of a
   * production, treat it as an implicit terminal (matches token by name). */
  const definedProductions = new Set(productions.keys());
  const implicitTerminals = new Set();

  function collectRefs(rule) {
    if(!rule) return;
    if(rule instanceof GNonTerminal) {
      if(!definedProductions.has(rule.name)) implicitTerminals.add(rule.name);
      return;
    }
    if(rule.children) for(const c of rule.children) collectRefs(c);
    if(rule.child) collectRefs(rule.child);
  }

  for(const alts of productions.values()) for (const a of alts) collectRefs(a);

  function promoteRefs(rule) {
    if(!rule) return rule;
    if(rule instanceof GNonTerminal) {
      if(implicitTerminals.has(rule.name)) return new GTerminal(rule.name, rule.name);
      return rule;
    }
    if(rule.children) rule.children = rule.children.map(promoteRefs);
    if(rule.child) rule.child = promoteRefs(rule.child);
    return rule;
  }

  for(const alts of productions.values()) for (let i = 0; i < alts.length; i++) alts[i] = promoteRefs(alts[i]);

  for(const t of implicitTerminals) declaredTokens.add(t);

  /* ---- assemble the Grammar ---- */
  const grammar = new Grammar(startName);
  grammar.declaredTokens = declaredTokens;

  for(const [name, alts] of productions) {
    let rule;
    if(alts.length === 1) rule = alts[0];
    else rule = new GAlternatives(alts);
    grammar.define(name, rule);
  }

  return grammar;

  /* ---- helper: parse one alternative (list of atoms), until |, ;, or EOF ---- */
  function parseAlternative() {
    const atoms = [];
    while(pos < tokens.length) {
      const p = peek();
      if(!p) break;
      if(p.type === 'bar' || p.type === 'r_pipe' || p.lexeme === '|') break;
      if(p.type === 'semi' || p.type === 'r_semi' || p.lexeme === ';') break;
      if(p.type === 'p_newline' || p.type === 'd_newline' || p.type === 'l_newline') {
        pos++;
        continue;
      }

      if(p.type === 'lbrace' || p.type === 'action' || p.type === 'cstart') {
        /* skip a balanced brace block – it's semantic action code */
        skipBraceBlock();
        continue;
      }
    ...
    if(atoms.length === 0) return new GEmpty();
    if(atoms.length === 1) return atoms[0];
    return new GSequence(atoms);
  }
  ...
  function parseAtom() {
    ...
    if(tok.type === 'lparen') {
      pos++;
      const inner = [parseAlternative()];
      while(pos < tokens.length) {
        const p = peek();
        if(p && (p.type === 'bar' || p.lexeme === '|')) {
          pos++;
          inner.push(parseAlternative());
        } else break;
      }
      if(peek()?.type === 'rparen') pos++;
      base = inner.length === 1 ? inner[0] : new GAlternatives(inner);
    } else if(tok.type === 'literal' || tok.type === 'char' || tok.type === 'chars') {
      pos++;
      const lex = tok.lexeme.slice(1, -1); /* strip quotes */
      base = new GTerminal(lex, JSON.stringify(lex));
      declaredLiterals.set(lex, base);
    } else if(tok.type === 'identifier' || tok.type === 'r_identifier') {
      pos++;
      if(declaredTokens.has(tok.lexeme)) base = new GTerminal(tok.lexeme, tok.lexeme);
      else base = new GNonTerminal(tok.lexeme);
    } else {
      return null;
    }
    ...
    while(pos < tokens.length) {
      const p = peek();
      if(!p) break;
      if(p.type === 'question' || p.lexeme === '?') {
        pos++;
        base = new GOptional(base);
      } else if(p.type === 'asterisk' || p.lexeme === '*') {
        pos++;
        base = new GZeroOrMore(base);
      } else if(p.type === 'plus' || p.lexeme === '+') {
        pos++;
        base = new GOneOrMore(base);
      } else break;
    }
    return base;
  }
  ...
  function skipBraceBlock() {
    ...
    while(pos < tokens.length) {
      const p = peek();
      if(!p) break;
      if(p.type === 'lbrace' || p.type === 'action' || p.type === 'cstart' || p.lexeme === '{') {
        depth++;
        pos++;
      } else if(p.type === 'rbrace' || p.lexeme === '}') {
        pos++;
        if(--depth <= 0) break;
      } else {
        pos++;
