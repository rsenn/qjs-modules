...
import Parser, { DumpToken } from '../parser.js';
...
class EBNFParser extends Parser {
  constructor(grammar) {
    ...
    this.lexer.handler = (arg, tok) => {
      ...
      let error = new Error(
        `Unmatched token at ${arg.loc} char='${BNFLexer.escape(line[index])}' section=${this.section} state=${
  ...
  error(tok, message) {
    const { lexer } = this;
    return new Error(
      `${tok.loc} ${tok.type} '${BNFLexer.escape(tok.lexeme)}' ${message}\n${lexer.currentLine()}\n${
