...
import Grammar, {
  Alternatives as GAlternatives,
  Empty as GEmpty,
  NonTerminal as GNonTerminal,
  OneOrMore as GOneOrMore,
  Optional as GOptional,
  ParseNode,
  Sequence as GSequence,
  Terminal as GTerminal,
  ZeroOrMore as GZeroOrMore,
  alt,
  empty,
  many,
  opt,
...
export function buildGrammar(source, filename) {
  ...
  while(pos < tokens.length) {
    ...
    const colonTok = peek();
    if(!(colonTok && (colonTok.type === 'colon' || colonTok.type === 'r_colon' || colonTok.lexeme === ':'))) throw new Error(`${colonTok?.loc}: expected ':' after '${name}', got '${colonTok?.lexeme}'`);
    pos++;
