...
function partialDeepEqual(actual, expected) {
  ...
  if(Array.isArray(expected)) {
    ...
    for(let i = 0; i < expected.length; i++) if(!partialDeepEqual(actual[i], expected[i])) return false;

    return true;
  }

  for(const key of Reflect.ownKeys(expected)) if(!(key in actual) || !partialDeepEqual(actual[key], expected[key])) return false;

  return true;
}

export function partialDeepStrictEqual(actual, expected, message) {
  if(!partialDeepEqual(actual, expected))
    throw new AssertionError(
      `partialDeepStrictEqual`,
      message ?? `Expected "actual" to be a partial deep-equal subset of:\x1b[0m\n\n${inspect(expected, { colors: false })}\n\nbut got\n\n${inspect(actual, { colors: false })}`,
    );
}
