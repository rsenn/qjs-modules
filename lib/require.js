...
function lookupModule(path) {
  let fstat = statPath(path);

  debug(`lookupModule# Looking for ${path}`);
  // Path found
  if(fstat.isFile) {
    debug(`lookupModule# Found module file`);
    return path;
  }

  // Path not found
  if(fstat.errno) {
    debug(`lookupModule# Not found module file`);
    // Try with '.js' extension
    if(!path.endsWith('.js') && statPath(`${path}.js`).isFile) {
  ...
  debug(`lookupModule# Path is a directory, trying options...`);
  // Try with package.json for NPM or YARN modules
  if(statPath(`${path}/package.json`).isFile) {
    debug(`lookupModule# It has package.json, looking for main script...`);
    let _pkg = JSON.parse(std.loadFile(`${path}/package.json`));
    if(_pkg &&
      Object.keys(_pkg).indexOf('main') !== -1 &&
      _pkg.main !== '' &&
      statPath(`${path}/${_pkg.main}`).isFile
    ) {
      _tryOthers = false;
  ...
  // Try other options
  if(_tryOthers && statPath(`${path}/index.js`).isFile) {
    _tryOthers = false;
    _path = `${path}/index.js`;
    debug(`lookupModule# Found package index.js file`);
  }
  if(_tryOthers && statPath(`${path}/main.js`).isFile) {
    _tryOthers = false;
    _path = `${path}/main.js`;
  ...
  debug(`lookupModule# Found module file: ${_path}`);
