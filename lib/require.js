...
let debug = arg => fs.writeFileSync(1, arg + '\n');

let debugOptions = process.env['DEBUG'];
if(typeof debugOptions == 'undefined' || debugOptions.split(',').filter(n => n == 'require').length == 0)
  debug = function() {};
...
const moduleStack = new Stack(
  value => (
    (globalThis.module = value), value?.exports ? (globalThis.exports = value.exports) : delete globalThis.exports
  ),
  () => globalThis.module
);
...
class CJSModule {
  constructor(id, m) {
    ...
    this._loaded = false;
    this.exports = {};
    debug(`CJSModule.constructor(${id}, ${m})`);
  }

  load() {
    const { id, path } = this;
    debug(`CJSModule.load() id=${id} path=${path}`);

    let ctx = { exports: {} };

    if((ctx.exports = requireModule(id))) {
      this.exports = ctx.exports;
      this._loaded = true;
      return true;
    }
    throw new Error(`requireModule(${id}) = ${ctx.exports}`);
    ...
    let _script = fs.readFileSync(__file, 'utf-8');
...
function memoize(fn, cache = new Map()) {
  let r;
  return (n, ...rest) => cache.get(n) ?? (cache.set(n, (r = fn(n, ...rest))), r);
}
...
function statPath(path) {
  ...
  let ret = {
    errno: err,
    isFile: fstat && fstat.mode & fs.constants.S_IFREG && true,
    isDir: fstat && fstat.mode & fs.constants.S_IFDIR && true
  };
...
function loadDSO(m) {
  ...
  if(globalThis.requireModule) return globalThis.requireModule(m);
...
/*function lookupPackage(name) {
  ...
  let json = fs.readFileSync(pkg + '/package.json', 'utf-8');
  let data = JSON.parse(json ?? '{}');
  if(data.main)
    return data.main.startsWith('.') ? pkg + data.main.slice(1) : pkg + '/' + data.main;
}*/

function createModule(m, _path) {
  debug(`createModule[${moduleStack.size}]# Module ${m}`);
  const [id, err] = fs.realpathSync(_path);
  if(err) {
  ...
  debug(`createModule[${moduleStack.size}]# id ${id}`);
  if(modules.hasOwnProperty(id)) {
    module = modules[id];
  } else {
    module = new CJSModule(m, _path);
    modules[id] = module;
    required.push(m);
    let result = module.load();
    required.pop();
    if(result !== true) {
      debug(`createModule[${moduleStack.size}]# error`, result);
      throw result;
      return;
    }
  }
  debug(`createModule[${moduleStack.size}]# success path:`, m);
  return module;
}
...
function lookupModule(m) {
  let paths, fstat, module_path;

  debug(`lookupModule# Looking for ${m}`);

  fstat = statPath((module_path = locateModule(m)));
  // Path found
  if(fstat.isFile) {
    debug(`lookupModule# Found module file '${module_path}'`);
    return module_path;
  }
  ...
  if(statPath(`${m}/package.json`).isFile) {
    ...
    let pkg = JSON.parse(fs.readFileSync(`${m}/package.json`, 'utf-8'));
...
export function require(m) {
  ...
  const { filename: file = process.argv[1] } = require;
  ...
  if(typeof file == 'undefined') {
    ...
    debug(
      `\x1b[1;31mrequire('${m}')\x1b[0m# scriptList=${[...scriptList]} scriptFile=${scriptFile} __dirname=${__dirname}`
    );
  }
  ...
  try {
    let _module = createModule(m, _path);
