...
export function* parseSelectors(s) {
  ...
  while((match = re.exec(s))) {
    ...
    /* console.log('parseSelector', match.index);
    console.log('parseSelector', re.lastIndex);
    console.log('parseSelector', { s, str, capture });*/
    let sel;
    ...
    if(str[0] == ':') {
      ...
      //console.log('parseSelector(2)', str, capture);
      const [, name, , value] = capture;
      //console.log('parseSelector(3)', { name, value }, capture.length);
      sel = value !== undefined ? AttributeSelector(name, value) : AttributeSelector(name);
      //console.log('parseSelector(4)', { sel });
    } else {
  ...
  if(selectors.length > 1) yield Predicate.or(...selectors);
