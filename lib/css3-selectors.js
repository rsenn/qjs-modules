...
function SimpleSelector(token) {
  switch (token.type) {
    ...
    /* unlike the exported Predicate-based constructors, the parser emits
       accessor closures here, so that attributes also match on wrapped
       dom.js elements (where attributes[name] is an Attr object) */
    case 'id': {
      const { name } = token;

      return e => getAttribute(e, 'id') === name;
    }

    case 'class': {
      const re = new RegExp(`(^|\\s)${escapeRegExp(token.name)}(\\s|$)`);

      return e => {
        const v = getAttribute(e, 'class');
    ...
    case 'attribute': {
      ...
      if(token.operator == undefined) return e => hasAttribute(e, name);

      const value = escapeRegExp(token.value ?? '');
      const flags = /[iI]/.test(token.caseSensitive ?? '') ? 'i' : '';
      const source = {
        '=': `^${value}$`,
        '~=': `(^|\\s)${value}(\\s|$)`,
        '|=': `^${value}(-|$)`,
      ...
      if(source == undefined) throw new Error(`ERROR parsing CSS selector: No such attribute operator '${token.operator}'`);
...
export function* parseSelectors(s) {
  for(const steps of parseSelectorList(s)) {
    ...
    for(let k = 0; k < steps.length; ) {
      ...
      while(j < steps.length && (steps[j].combinator == '+' || steps[j].combinator == '~')) j++;

      if(j == k + 1) {
        for(const part of steps[k].parts) values.push(SimpleSelector(part));
      } else {
        /* a run of sibling steps becomes one chained closure anchored at its rightmost compound */
        let chain = CompoundPredicate(steps[k].parts);

        for(let m = k + 1; m < j; m++) chain = CombinatorSelector(steps[m].combinator, chain, CompoundPredicate(steps[m].parts));
