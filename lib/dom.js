...
import { className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, mapObject, getset, getter, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, lazyProperties, memoize, modifier, quote, range, properties, } from 'util';
...
extend(NamedNodeMap, {
  ...
  inspect(obj) {
    ...
    for(let { name, value } of [...obj]) {
      if(s) s += ' ';
      s += `\x1b[1;35m${name}\x1b[0m=\x1b[1;36m"${value}"\x1b[0m`;
    }
    return s;
  },
},
