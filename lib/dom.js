...
import { className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, getset, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, lazyProperties, memoize, modifier, quote, range, properties, } from 'util';
...
function setParentOwner(obj, parentOrOwner) {
  if(!isObject(parentOrOwner) || 'nodeType' in parentOrOwner) parentNodes(obj, parentOrOwner);
  else ownerElements(obj, parentOrOwner);
}
...
export class Interface {
  ...
  get isConnected() {
    return isObject(Node.owner(this));
  }
  ...
  cloneNode(deep = true) {
    ...
    /*const p = Parser.for(this);
    const d = p.parseFromString(this.outerHTML);
...
export class Node {
  ...
  /*static owner(node) {
    ...
    if((owner = parentNodes(node)) && Array.isArray(owner.children)) return owner.children;
  }*/
...
export class HTMLCollection {
  constructor(obj, owner, pred = e => true) {
    ...
    rawNode(this, obj);
    setParentOwner(ownerElements, this);
    ...
    rawNode(coll, obj);
    setParentOwner(ownerElements, coll);
...
export function NamedNodeMap(obj, owner) {
  ...
  rawNode(nodeMap, obj);
  setParentOwner(ownerElements, nodeMap);
...
export class CSSStyleDeclaration {
  constructor(style, owner) {
    ...
    for(const lnk of [this, obj]) {
      ...
      if(isObject(owner)) setParentOwner(ownerElements, lnk);
    }
...
function MakeCache(ctor, store = new WeakMap()) {
  ...
  return (key, ...args) => {
    ...
    setParentOwner(ownerElements, value);
    return value;
  };
}
