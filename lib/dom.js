...
export class Interface {
  ...
  querySelector(s) {
    if(isString(s)) selectors = [...parseSelectors(s)];
  ...
  *querySelectorAll(s) {
    ...
    for(const sel of parseSelectors(s)) {
      ...
      for(let path of iterate(raw, sel ?? (() => true), RETURN_PATH, TYPE_OBJECT, ['children'])) {
        console.log('*querySelectorAll', console.config({ compact: true }), { path });
        const node = applyPath(path, this);

        //for(let node of queryAll(raw, [sel], path => applyPath(path, this))) {
        //console.log('*querySelectorAll', console.config({ compact: true }), node);
