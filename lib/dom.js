...
import { types, className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, mapObject, getset, getter, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, lazyProperties, memoize, modifier, quote, range, properties, isPrototypeOf, } from 'util';
...
define(Factory, {
  type(node) {
    ...
    return factory.entities(node)?.type;
  },
  for(node) {
    const factory = this.get(node);
...
export class Node {
  ...
  static depth(node, pred = (node, depth) => true) {
    return this.hier(node, pred).length;
  }

  static hier(node, pred, forward, t) {
    const r = [],
      p = [],
    ...
    for(const n of this.up(node)) {
      ...
      if(raw && prev) {
        ...
        let [k, v] = entries.find(([k, v]) => v == prev);
        if(isNumeric(k)) k = +k;
        p.unshift(k);
      }
  ...
  static path(node, path) {
    if(node.ownerElement) {
      const child = {
        ...
        Attr: ['attributes', node.name],
      }[node.constructor.name];

      if(child.reduce((acc, key) => acc[key], node.ownerElement)) return Node.path(node.ownerElement).concat(child);
    }

    path ??= [];

    const [first] = Node.hier(node, undefined, undefined, (n, p) => [...p]);
...
define(
  ...
  nonenumerable({
    [Symbol.inspect](depth, opts) {
      ...
      const attrs = NamedNodeMap.inspect(attributes, depth + 1, opts)
        //.slice(1, -1)
        .trim();
      let str = `<${tagName}`;
      if(attrs) str += ' ' + attrs;
...
export class Comment extends Node {
  ...
  [Symbol.inspect](depth, opts) {
    return `${className(this) || 'Comment'} \x1b[38;2;184;0;234m${this.data}\x1b[0m`;
  }
