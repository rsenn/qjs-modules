...
import { isInstanceOf, types, className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, mapObject, getset, getter, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, lazyProperties, memoize, modifier, quote, range, properties, isPrototypeOf, weakMapper, } from 'util';
...
export class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    const observers = MutationObserver.observersFor(this);

    if(observers)
      for(let observer of observers) {
        observer.event(MutationRecord.attribute(name, null, this, oldValue));
      }
  }
...
export class MutationRecord {
  ...
  static characterData(target, oldValue) {
    return new MutationRecord({ type: 'attribute', target, oldValue });
  }
