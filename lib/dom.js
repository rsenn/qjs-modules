...
define(URL.prototype, { [Symbol.toStringTag]: 'URLSearchParams' });
...
export class URL {
  constructor(url, base) {
    const re = /^([^:]+:)\/\/([^:@]+(|:[^@]*)@|)([^:/]+)(:[^/:]*|)([^\?#]*)(\?[^#]*|)(#.*|)/;
    const [, protocol, user, pass, hostname, port, pathname, search, hash] = url.match(re);

    Object.assign(this, {
      protocol,
      username: user ? user.replace(/:.*/g, '') : undefined,
      password: pass ? pass.slice(1) : undefined,
      hostname,
      port: port ? +port.slice(1) : undefined,
      pathname,
  ...
  get host() {
    return this.hostname + (this.port === undefined ? '' : ':' + this.port);
  }
  get auth() {
    return this.username ? this.username + (this.password === undefined ? '' : ':' + this.password) : undefined;
  }
  get origin() {
...
define(URL.prototype, { [Symbol.toStringTag]: 'URL', protocol: null, hostname: null, port: null, pathname: null, searchParams: null, hash: null });
