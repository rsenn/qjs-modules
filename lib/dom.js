  ...
  get lastElementChild() {
    ...
    if(children?.length)
      for(let i = children.length - 1; i >= 0; i--)
        if(isObject(children[i]) && 'tagName' in children[i]) return Element.cache(children[i], this);
  }
...
const TextValue = gettersetter(new WeakMap());
const TextCache = gettersetter(new WeakMap());
  ...
  constructor(key, owner) {
    ...
    let get, set;
     if(owner) {
      ...
      TextCache(this, { text: key });
      get = () => TextCache(this).text;
      set = value => (TextCache(this).text = value);
    }
    TextValue(this, modifier(get, set));
    //console.log('Text', get());
  }

  get data() {
    return TextValue(this)(value => value);
  }
