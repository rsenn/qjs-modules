...
export class History {
  ...
  #dispatchHashChange(oldHash, newHash) {
    const event = new HashChangeEvent('hashchange', {
      oldURL: this.#window.location.href.replace(/#.*$/, '') + oldHash,
...
export class PopStateEvent extends Event {
  ...
  constructor(type, eventInitDict = {}) {
    ...
    this.#oldState = eventInitDict.oldState ?? null;
  }
    ...
    if(this.#startContainer === this.#endContainer) {
      if(this.#startContainer.nodeType === Node.TEXT_NODE) {
        ...
        this.#startContainer.data =
          data.slice(0, this.#startOffset) + data.slice(this.#endOffset);
      } else {
...
export class Selection {
  ...
  get isCollapsed() {
    return (
      this.#anchorNode === this.#focusNode && this.#anchorOffset === this.#focusOffset
    );
  }
