...
export class Parser {
  ...
  parseFromString(str, file) {
    ...
    let doc = factory.Document.new(data, factory);
...
export class Interface {
  ...
  querySelectorAll(...selectors) {
    ...
    let result = [...query(node, selectors)];
    return result.map(p => applyPath(p, this));
  }
}
...
export class Element extends Node {
  ...
  get innerText() {
    let raw = Node.raw(this);
    let texts = [];
    for(let [value, path] of deep.iterate(raw, (n, k) => typeof n == 'string' && k[k.length - 2] == 'children')) {
...
export class Document extends Element {
  ...
  [inspectSymbol](depth, opts) {
    ...
    return `\x1b[1;31mDocument\x1b[0m ${tagName} attributes: ${attributes[inspectSymbol](
      depth + 1,
      opts
    )} children: ${children.reduce((acc, c) => acc + c[inspectSymbol](depth + 2, opts), '')}>`;
  }
}
