...
function* walk(root) {
  ...
  for(let path of iterate(raw, () => true, RETURN_PATH, TYPE_OBJECT | TYPE_STRING, ['children'])) yield path.reduce((o, k) => o[k], root);
}
...
export class Interface {
  ...
  contains(other) {
    for(const el of walk(this.ownerDocument)) {
      if(el == other) return true;
      if(el.isSameNode(other)) return true;
    }
    return false;
  }
