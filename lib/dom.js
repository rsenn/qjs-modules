...
import { isObject, define, quote, range, assert, memoize, getset, modifier, gettersetter, arrayFacade } from 'util';
...
export class Parser {
  parseFromString(str, file, ...rest) {
    let data = readXML(str, file, ...rest);
  ...
  parseFromFile(file, ...rest) {
    let xml = readFileSync(file);
    return this.parseFromString(xml, file, ...rest);
  }
}
...
function GetNode(obj, owner) {
  ...
  if(!type || !type.cache) throw new Error(`No such node type for ${obj[inspectSymbol]()}`);
