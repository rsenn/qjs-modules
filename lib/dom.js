...
export class Interface {
  ...
  getRootNode() {
    for(let parent, node = Node.ownerOrParent(this); node; node = parent) if(!(parent = Node.ownerOrParent(node))) return node;
  }
...
export class Text extends Node {
  ...
  constructor(key, owner) {
    ...
    //console.log('Text.constructor', { key, owner });

    if(owner) {
      textValues(this, Text.own(owner, key));
    } else {
      if(typeof key == 'string') Text.store(this, key);

      textValues(this, (...args) => Text.store(this, ...args));
    }
  }
  ...
  static own(owner, key) {
    ...
    return modifier([() => raw[key] ?? '', value => (raw[key] = value)]);
  }
