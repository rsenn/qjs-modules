  ...
  get nextSibling() {
    ...
    if(parentNode.hasChildNodes()) {
      ...
      if(index != -1 && children[index + 1]) {
        const owner = ownerElements(this);

        DEBUG('get nextSibling', { children, index, owner });

        return GetNode(children[index + 1], owner);
      }
    }
  }
  ...
  get previousSibling() {
    ...
    if(parentNode.hasChildNodes()) {
      ...
      if(index != -1 && children[index - 1]) {
        ...
        DEBUG('get previousSibling', { children, index, owner });
...
export class Node extends Interface {
  ...
  static owner(node) {
    let tmp;
    if((tmp = ownerElements(node))) return tmp;
  }
...
export function NamedMap(node, get, keys) {
  ...
  if(isString(get)) {
    const key = get;
    get = n => ((n = raw.children.find(e => e.attributes[key] == n)), n ? GetNode(n, node) : n);
