...
export function Factory(types = Prototypes()) {
  ...
  if(new.target) {
    ...
    for(let i = 0; i < EntityNames.length; i++) {
      ...
      ret[name] = { new: (...args) => types(name, ...args), cache: (...args) => types.cache(name, ...args) };
    }
...
export class Interface {
  ...
  get childNodes() {
    ...
    return Factory(this).createNodeList((raw.children ??= []), this, NodeList);
  }
...
export class Element extends Node {
  ...
  get children() {
    ...
    return Factory.for(this).createNodeList((raw.children ??= []), this);
  }
