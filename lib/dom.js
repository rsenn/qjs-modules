...
export class Interface {
  ...
  querySelector(...selectors) {
    ...
    const gen = query(Node.raw(this), selectors);
  ...
  querySelectorAll(...selectors) {
    ...
    return query(Node.raw(this), selectors, p => applyPath(p, this));
  }
}
...
function MakeCache(ctor, store = new WeakMap()) {
  ...
  return (key, ...args) => {
    ...
    ownerElements.set(value, args[0]);
    return value;
  };
}
...
export class Element extends Node {
  ...
  [inspectSymbol](depth, opts) {
    ...
    const attrs = attributes[inspectSymbol](depth + 1, opts)
      .slice(1, -1)
      .trim();
    ...
    try {
      const tmp = children[inspectSymbol](depth + 1, opts);
...
const stylemap = gettersetter(new WeakMap());

export class CSSStyleDeclaration {
  constructor(attributes, owner) {
    const impl = {
      styles: parseStyle(attributes?.style ?? ''),
      get(key) {
        return key in this.styles ? this.styles[key] : '';
      },
      set(key, value) {
        this.styles[key] = value;
        attributes.style = formatStyle(this.styles);
      },
      remove(key) {
    ...
    rawNode.set(this, attributes);
    rawNode.set(obj, attributes);
  ...
  setProperty(k, v) {
    stylemap(this).set(k, v);
  }

  item(index) {
    let i = 0;

    for(let k of stylemap(this).keys()) if(i++ == index) return k;
  }

  getPropertyValue(key) {
    return stylemap(this).get(key);
  }
  ...
  removeProperty(key) {
    return stylemap(this).remove(key);
  }

  get cssText() {
    return Node.raw(this).style;
  }

  set cssText(value) {
    Node.raw(this).style = formatStyle((stylemap(this).styles = parseStyle(value)));
  }
  ...
  [inspectSymbol](depth, opts) {
    ...
    return `\x1b[1;31mCSSStyleDeclaration\x1b[0m {${spacing}${formatStyle(stylemap(this).styles, ';', spacing, indent)}${spacing}}`;
  }
}
