...
/* Builds a per-traversal predicate testing whether every value in `values`
 * matches some ancestor-or-self of the current node, for descendant/child
 * combinator selectors (css3-selectors.js flattens those steps into
 * `values`). Consecutive predicate calls during a deep-module DFS share
 * most of their path with the previous call, so match results and node
 * lookups are cached per depth and only recomputed for the differing
 * suffix, instead of re-walking the whole ancestor chain from the
 * document root on every visited node. */
function ancestorMatcher(raw, values) {
  if(values.length == 1) {
    const v = values[0];
    return node => (v(node) ? YIELD_NO_RECURSE : RECURSE);
  }

  let prevPath = [];
  let stack = [raw];
  let matched = [values.map(() => false)];

  return (node, path) => {
    const len = path.length;
    let k = 0;

    while(k < len && k < prevPath.length && path[k] === prevPath[k]) k++;
    ...
    for(let i = k; i < len; i++) {
      ...
      stack.push(value);
      matched.push(key == 'children' ? prev : prev.map((m, j) => m || values[j](value)));
    }

    prevPath = [];
    for(let i = 0; i < len; i++) prevPath.push(path[i]);

    return matched[matched.length - 1].every(m => m) ? YIELD_NO_RECURSE : RECURSE;
  };
}
...
export class Event {
  ...
  constructor(type, { bubbles, cancelable, composed } = {}) {
    this.#type = type;
...
export class UIEvent extends Event {
  ...
  constructor(type, { view, detail, ...rest } = {}) {
    super(type, rest);
    this.#view = view ?? null;
...
export class MouseEvent extends UIEvent {
  ...
  constructor(type, { screenX, screenY, clientX, clientY, ctrlKey, shiftKey, altKey, metaKey, button, buttons, relatedTarget, ...rest } = {}) {
    super(type, rest);
...
export class KeyboardEvent extends UIEvent {
  ...
  constructor(type, { key, code, location, ctrlKey, shiftKey, altKey, metaKey, repeat, isComposing, charCode, keyCode, ...rest } = {}) {
    super(type, rest);
    this.#key = key ?? '';
    this.#code = code ?? '';
...
export class FocusEvent extends UIEvent {
  ...
  constructor(type, { relatedTarget, ...rest } = {}) {
    super(type, rest);
    this.#relatedTarget = relatedTarget ?? null;
  }
...
export class InputEvent extends UIEvent {
  ...
  constructor(type, { data, inputType, isComposing, ...rest } = {}) {
    super(type, rest);
    this.#data = data ?? null;
...
export class WheelEvent extends MouseEvent {
  ...
  constructor(type, { deltaX, deltaY, deltaZ, deltaMode, ...rest } = {}) {
    super(type, rest);
    this.#deltaX = deltaX ?? 0;
    this.#deltaY = deltaY ?? 0;
    this.#deltaZ = deltaZ ?? 0;
    this.#deltaMode = deltaMode ?? 0;
  }
...
export class TouchEvent extends UIEvent {
  ...
  constructor(type, { touches, targetTouches, changedTouches, ctrlKey, shiftKey, altKey, metaKey, ...rest } = {}) {
    super(type, rest);
    this.#touches = touches ?? new TouchList();
    this.#targetTouches = targetTouches ?? new TouchList();
    this.#changedTouches = changedTouches ?? new TouchList();
    this.#ctrlKey = !!ctrlKey;
    this.#shiftKey = !!shiftKey;
    this.#altKey = !!altKey;
    this.#metaKey = !!metaKey;
...
export class PointerEvent extends MouseEvent {
  ...
  constructor(type, { pointerId, width, height, pressure, tiltX, tiltY, pointerType, isPrimary, ...rest } = {}) {
    super(type, rest);
...
export class Interface extends EventTarget {
  ...
  cloneNode(deep = true) {
    // Handle Text nodes specially - they're just strings, not objects
    if(this.nodeType === TEXT_NODE) {
  ...
  appendChild(node) {
    ...
    if(isInstanceOf(Text, node)) {
      // Set up the getter/setter AFTER pushing to children array so it points to the correct index
      // Pass the children array directly to Text.own, not the parent object
      textValues(node, Text.own(children, children.length - 1));
      // Store the Text node in the cache so that childNodes[index] returns the same node
      Text.cache(children.length - 1, this.childNodes, node);
    }
  ...
  removeChild(node) {
    const children = (Node.raw(this).children ??= []);
    const rawChild = isNode(node) ? Node.raw(node) : node;
    let index = typeof rawChild === 'number' ? rawChild : children.indexOf(rawChild);
    if(index == -1 || index >= children.length) throw new Error(`Node.removeChild no such child!`);
  ...
  querySelector(s) {
    ...
    try {
      for(let sel of parseSelectors(s)) {
        ...
        let path = find(
          raw,
          sel ? ancestorMatcher(raw, values) : e => !/^[!?]/.test(e.tagName),
...
export class Node extends Interface {
  ...
  static raw(node) {
    ...
    const r = rawNode(node);
    if(typeof r === 'number' && node.nodeType === TEXT_NODE) return textValues(node)?.() ?? '';
    return r;
  }
  ...
  static document(node) {
    let doc = node;
    while(doc) {
      if(doc.nodeType == DOCUMENT_NODE) break;
      doc = parentNodes(doc) ?? ownerElements(doc);
    }
...
export class NodeList {
  constructor(obj, owner) {
    ...
    /* owner is fixed for this NodeList's lifetime, so the factory it
     * resolves to (always the owning document's single Factory) is too -
     * resolving it once here instead of on every indexed access avoids
     * repeating Factory.for()'s Node.document() walk per element read. */
    ...
    const nodeList = new Proxy(this, {
      get: (target, prop, receiver) => {
        ...
        if(isIndex(prop)) {
          let node;
          if(prop in obj) {
            node = GetNode(obj[prop], nodeList, factory);
...
extend(DOMRectReadOnly.prototype, nonenumerable({ [Symbol.toStringTag]: 'DOMRectReadOnly' }));
...
export class DOMRect extends DOMRectReadOnly {
  ...
  set y(value) {
    Object.defineProperty(this, 'y', { value, writable: true, configurable: true });
  }
  ...
  set width(value) {
    Object.defineProperty(this, 'width', { value, writable: true, configurable: true });
  }
  ...
  set height(value) {
    Object.defineProperty(this, 'height', { value, writable: true, configurable: true });
  }
...
extend(DOMRect.prototype, nonenumerable({ [Symbol.toStringTag]: 'DOMRect' }));
...
export class Element extends Node {
  ...
  getBoundingClientRect() {
    const raw = Node.raw(this);
    const rect = raw.getBoundingClientRect?.() ?? { x: 0, y: 0, width: 0, height: 0 };
...
export class HTMLElement extends Element {
  ...
  get offsetWidth() {
    return Node.raw(this).offsetWidth ?? 0;
  }

  set offsetWidth(value) {
    Node.raw(this).offsetWidth = value;
  }

  get offsetHeight() {
    return Node.raw(this).offsetHeight ?? 0;
  }

  set offsetHeight(value) {
    Node.raw(this).offsetHeight = value;
  }

  get offsetTop() {
    return Node.raw(this).offsetTop ?? 0;
  }

  set offsetTop(value) {
    Node.raw(this).offsetTop = value;
  }

  get offsetLeft() {
    return Node.raw(this).offsetLeft ?? 0;
  }

  set offsetLeft(value) {
    Node.raw(this).offsetLeft = value;
  }

  get offsetParent() {
    return Node.raw(this).offsetParent ?? null;
  }

  set offsetParent(value) {
    Node.raw(this).offsetParent = value;
  }

  get clientWidth() {
    return Node.raw(this).clientWidth ?? 0;
  }

  set clientWidth(value) {
    Node.raw(this).clientWidth = value;
  }

  get clientHeight() {
    return Node.raw(this).clientHeight ?? 0;
  }

  set clientHeight(value) {
    Node.raw(this).clientHeight = value;
  }

  get clientTop() {
    return Node.raw(this).clientTop ?? 0;
  }

  set clientTop(value) {
    Node.raw(this).clientTop = value;
  }

  get clientLeft() {
    return Node.raw(this).clientLeft ?? 0;
  }

  set clientLeft(value) {
    Node.raw(this).clientLeft = value;
  }

  get scrollWidth() {
    return Node.raw(this).scrollWidth ?? 0;
  }

  set scrollWidth(value) {
    Node.raw(this).scrollWidth = value;
  }

  get scrollHeight() {
    return Node.raw(this).scrollHeight ?? 0;
  }

  set scrollHeight(value) {
    Node.raw(this).scrollHeight = value;
  }

  get scrollTop() {
    return Node.raw(this).scrollTop ?? 0;
  }

  set scrollTop(value) {
    Node.raw(this).scrollTop = value;
  }

  get scrollLeft() {
    return Node.raw(this).scrollLeft ?? 0;
  }

  set scrollLeft(value) {
    Node.raw(this).scrollLeft = value;
  }
}
...
function MakeCache2(ctor, store = new WeakMap()) {
  ...
  return (id, owner, existingNode) => {
    const textList = cache(owner);
    if(isNumeric(id)) id = +id;
    if(existingNode) {
      textList[id] = existingNode;
    } else {
...
export class Navigator {
  ...
  constructor(options = {}) {
    ...
    this.#platform = options.platform ?? (typeof process !== 'undefined' ? process.platform : undefined) ?? 'unknown';
...
export class History {
  ...
  constructor(window) {
    ...
    this.#stack.push({ state: null, title: '', url: window.location.href });
    this.#index = 0;
  }
  ...
  get state() {
    return this.#index >= 0 && this.#index < this.#stack.length ? this.#stack[this.#index].state : null;
  }
  ...
  pushState(state, title, url) {
    ...
    console.log('this.#window.location.href', this.#window.location.href);
    console.log('pushState', { url, resolvedUrl });

    // Capture old URL and hash before updating
    const oldUrl = this.#window.location.href;
    const oldHash = this.#window.location.hash;
    const newHash = new URL(resolvedUrl).hash;

    console.log('pushState', { oldHash, newHash });

    // Add new entry
    this.#stack.push({ state, title, url: resolvedUrl });
    this.#index = this.#stack.length - 1;

    // Update location
    this.#window.location.href = resolvedUrl;

    // Dispatch hashchange if hash changed
    if(oldHash !== newHash) {
      this.#dispatchHashChange(oldUrl, this.#window.location.href);
    }
  }
  ...
  replaceState(state, title, url) {
    ...
    this.#stack[this.#index] = { state, title, url: resolvedUrl };
    ...
    // Dispatch hashchange if hash changed
    if(oldHash !== newHash) {
      this.#dispatchHashChange(oldHash, newHash);
    }
  }
  ...
  go(delta = 0) {
    ...
    const oldEntry = this.#stack[this.#index];
    this.#index = newIndex;
    const newEntry = this.#stack[this.#index];

    // Update location
    this.#window.location.href = newEntry.url;

    // Check for hash change
    const oldHash = new URL(oldEntry.url).hash;
    const newHash = new URL(newEntry.url).hash;
    ...
    // Dispatch popstate
    this.#dispatchPopState(oldEntry.state);
  }

  #resolveUrl(url) {
    if(!url) return this.#window.location.href;
    try {
  ...
  #dispatchHashChange(oldUrl, newUrl) {
...
extend(History.prototype, nonenumerable({ [Symbol.toStringTag]: 'History' }));

export class PopStateEvent extends Event {
  #state;
  #oldState;

  constructor(type, { state, oldState, ...rest } = {}) {
    super(type, rest);
    this.#state = state ?? null;
    this.#oldState = oldState ?? null;
  }
...
extend(PopStateEvent.prototype, nonenumerable({ [Symbol.toStringTag]: 'PopStateEvent' }));

export class HashChangeEvent extends Event {
  #oldURL;
  #newURL;

  constructor(type, { oldURL, newURL, ...rest } = {}) {
    super(type, rest);
    this.#oldURL = oldURL ?? '';
    this.#newURL = newURL ?? '';
  }
...
extend(HashChangeEvent.prototype, nonenumerable({ [Symbol.toStringTag]: 'HashChangeEvent' }));
...
export class Window extends EventTarget {
  ...
  constructor(options = {}) {
    ...
    this.#history = options.history ?? new History(this);
...
export class DocumentFragment extends Node {
  constructor(children = []) {
    ...
    /* Unlike Document, a standalone fragment (new DocumentFragment(), not
     * document.createDocumentFragment()) has no owner document for
     * Factory.get() to fall back to - register directly with `factories`
     * rather than Factory.set(), which would try to also propagate to
     * Node.document(this) and throw ('in' on undefined) since a fragment
     * with no owner has none. */
    if(!factories(this)) factories(this, new Factory());
  }
...
export class Range {
  ...
  get commonAncestorContainer() {
    ...
    // Walk up from startContainer to find common ancestor
    let node = this.#startContainer;
    while(node) {
      if(this.#contains(node, this.#endContainer)) return node;
      node = node.parentNode;
    }
    return null;
  }
  ...
  setStart(node, offset) {
    if(!node) throw new TypeError('setStart: node is required');
    this.#startContainer = node;
    this.#startOffset = offset;

    // If start is after end, collapse to start
    if(this.#compareBoundaryPoints() > 0) {
  ...
  setEnd(node, offset) {
    if(!node) throw new TypeError('setEnd: node is required');
    this.#endContainer = node;
    this.#endOffset = offset;
  ...
  setStartBefore(node) {
    if(!node.parentNode) throw new Error('setStartBefore: node has no parent');
    this.setStart(node.parentNode, this.#indexOf(node));
  }

  setStartAfter(node) {
    if(!node.parentNode) throw new Error('setStartAfter: node has no parent');
    this.setStart(node.parentNode, this.#indexOf(node) + 1);
  }

  setEndBefore(node) {
    if(!node.parentNode) throw new Error('setEndBefore: node has no parent');
    this.setEnd(node.parentNode, this.#indexOf(node));
  }

  setEndAfter(node) {
    if(!node.parentNode) throw new Error('setEndAfter: node has no parent');
    this.setEnd(node.parentNode, this.#indexOf(node) + 1);
  }
  ...
  selectNode(node) {
    if(!node.parentNode) throw new Error('selectNode: node has no parent');
  ...
  selectNodeContents(node) {
    ...
    this.#startOffset = 0;
    this.#endContainer = node;
    this.#endOffset = this.#getNodeLength(node);
  }
  ...
  #compareBoundaryPoints() {
    ...
    // Simplified comparison - in real implementation this would be more complex
    return -1;
  }

  cloneContents() {
    const doc = this.#startContainer?.ownerDocument ?? this.#endContainer?.ownerDocument;
    const fragment = doc ? doc.createDocumentFragment() : new DocumentFragment();
    if(this.collapsed) return fragment;

    // Simplified implementation - clone the range content
    if(this.#startContainer === this.#endContainer) {
      if(this.#startContainer.nodeType === TEXT_NODE) {
        const text = this.#startContainer.data.slice(this.#startOffset, this.#endOffset);
  ...
  deleteContents() {
    ...
    if(this.#startContainer === this.#endContainer) {
      if(this.#startContainer.nodeType === TEXT_NODE) {
        ...
        this.#startContainer.data = data.slice(0, this.#startOffset) + data.slice(this.#endOffset);
      } else {
        const children = Array.from(this.#startContainer.childNodes);
        for(let i = this.#endOffset - 1; i >= this.#startOffset; i--) {
          if(children[i]) this.#startContainer.removeChild(children[i]);
        }
      }
    }
    this.collapse(true);
  ...
  insertNode(node) {
    if(!this.#startContainer) throw new Error('insertNode: range not initialized');
    ...
    if(this.#startContainer.nodeType === TEXT_NODE) {
      ...
      this.#startContainer.data = before;
      const newNode = new Text(after);
      this.#startContainer.parentNode.insertBefore(newNode, this.#startContainer.nextSibling);
      this.#startContainer.parentNode.insertBefore(node, newNode);
    } else {
      const refNode = this.#startContainer.childNodes[this.#startOffset] || null;
      this.#startContainer.insertBefore(node, refNode);
    }
  }
  ...
  detach() {
    // No-op in modern browsers, kept for compatibility
  }
...
extend(Range.prototype, nonenumerable({ [Symbol.toStringTag]: 'Range' }));
...
export class Selection {
  ...
  get type() {
    if(this.#ranges.length === 0) return 'None';
    if(this.isCollapsed) return 'Caret';
    return 'Range';
  }

  getRangeAt(index) {
    if(index < 0 || index >= this.#ranges.length) {
      throw new Error('getRangeAt: index out of range');
    }
    return this.#ranges[index];
  }

  addRange(range) {
    if(!(range instanceof Range)) throw new TypeError('addRange: argument must be a Range');
  ...
  removeRange(range) {
    ...
    if(index === -1) throw new Error('removeRange: range not in selection');
...
extend(Selection.prototype, nonenumerable({ [Symbol.toStringTag]: 'Selection' }));
