...
import { get, iterate, find, RETURN_PATH } from 'deep';
...
function query(root, selectors, t = (path, root) => path) {
  let path;
  for(let selector of selectors) if((path = find(root, selector, RETURN_PATH))) return t(path, root);
}
  ...
  if(new.target) {
    ...
    if(!isFunction(types)) {
      ...
      types.cache = (type, ...args) => {
        ...
        return (proto.constructor.cache ?? Node.makeCache((...a) => new proto.constructor(...a)))(...args);
      };
    }
...
export class Interface {
  ...
  appendChild(node) {
    ...
    Node.ownerElements(node, this.childNodes);
    Node.parentNodes(node, this);
  ...
  querySelectorAll(...selectors) {
    ...
    return queryAll(Node.raw(this), selectors, p => applyPath(p, this));
  }
}

export class Node {
  static mapper = gettersetter(new WeakMap());
  static parentNodes = gettersetter(new WeakMap());
  static ownerElements = gettersetter(new WeakMap());

  static makeCache(ctor, store = new WeakMap()) {
    ...
    return (key, ...args) => {
      ...
      this.ownerElements(value, args[0]);
      return value;
    };
  }

  constructor(obj, parent) {
    if(isObject(obj)) Node.mapper(this, obj);

    Node.parentNodes(this, parent);
  }
  ...
  static owner(node) {
    ...
    if((parent = Node.parentNodes(node)) && Array.isArray(parent.children)) return parent.children;

    const raw = Node.raw(node);

    if((parent = Node.parentNodes(raw)) && Array.isArray(parent.children)) return parent.children;

    if((owner = Node.ownerElements(node))) return owner;

    return Node.ownerElements(raw);
  }

  static parent(node) {
    checkNode(node);

    return Node.parentNodes(node);
  }
...
export class NodeList {
  static mapper = gettersetter(new WeakMap());
  static owner = gettersetter(new WeakMap());
  ...
  constructor(obj, owner) {
    ...
    NodeList.mapper(this, obj);
    NodeList.owner(this, owner);
    ...
    NodeList.mapper(nodeList, obj);
    NodeList.owner(nodeList, owner);
...
export function NamedNodeMap(obj, owner) {
  ...
  Node.mapper(this, obj);
  Node.ownerElements(this, owner);
  ...
  Node.mapper(nodeMap, obj);
  Node.ownerElements(nodeMap, owner);
...
export class Element extends Node {
  ...
  get innerText() {
    ...
    for(let [value, path] of iterate(Node.raw(this), (n, k) => isString(n) && k[k.length - 2] == 'children')) texts.push(value.replace(/<s+/g, ' '));
  ...
  static cache = Node.makeCache((obj, owner) => new Element(obj, owner));
...
export class Attr extends Node {
  ...
  get ownerElement() {
    return Node.ownerElements(this);
  }
...
export class Text extends Node {
  static #store = gettersetter(Node.mapper);
  ...
  constructor(key, owner) {
    ...
    Node.textValues(this, modifier(get, set));
  }

  get data() {
    return Node.textValues(this)(value => value);
  }
...
export class CSSStyleDeclaration {
  constructor(style, owner) {
    ...
    for(let lnk of [this, obj]) {
      styleImpl(lnk, impl);
      Node.mapper(lnk, style);

      if(isObject(owner)) Node.ownerElements(lnk, owner);
    }
