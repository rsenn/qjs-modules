...
export function NamedNodeMap(obj, owner) {
  ...
  const fn = isFunction(obj) ? obj : gettersetter(obj);
  const adapter = mapObject(fn);

  const isAttr = prop => isString(prop) && adapter.has(prop);
  const wrapAttr = (value, prop) => new Attr([(...args) => fn(prop, ...args), prop], nodeMap);
