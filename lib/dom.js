...
export class Node {
  ...
  querySelector(...selectors) {
    ...
    for(let path of deep.iterate(node, selector, deep.RETURN_PATH)) {
      result = applyPath(path, this);

      if(selectors.length) result = result.querySelector(...selectors);
  ...
  querySelectorAll(str) {
    ...
    let node = Node.raw(this);
    let result = deep.select(node, selector, deep.RETURN_PATH);
    return result ? result.map(path => applyPath(path, this)) : result;
  }
