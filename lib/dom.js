...
import { isObject, define, quote, range, assert, memoize, getset, modifier, gettersetter, arrayFacade, lazyProperties } from 'util';
...
const EntityNames = ['Document', 'Node', 'NodeList', 'Element', 'NamedNodeMap', 'Attr', 'Text', 'Comment', 'TokenList'];
...
function* query(root, selectors, base = []) {
  let selector = selectors.shift();
  let result = [];

  for(let path of deep.iterate(root, selector, deep.RETURN_PATH)) {
    let node = applyPath(path, root);

    if(selectors.length) yield* query(node, selectors, base.concat(path));
    else yield base.concat(path);
  }
}
...
export function NodeFactory(types = Prototypes()) {
  let ret;

  if(new.target) {
    ret = function NodeFactory(type) {
      if(typeof type == 'number') type = EntityNames[type];
      // if(typeof type == 'string') type = EntityType(type);

      return ret['create' + type];
    };
    ...
    for(let i = 0; i < EntityNames.length; i++) {
      ...
      ret['create' + name] = (...args) => types(name, ...args);
      ret['create' + name].cache = (...args) => types.cache(name, ...args);
    }

    return Object.setPrototypeOf(ret, NodeFactory.prototype);
  }

  return factories(types);
}

Object.setPrototypeOf(NodeFactory.prototype, Function.prototype);
...
export class Parser {
  parseFromString(str, file, factory = new NodeFactory()) {
    ...
    let doc = factory.createDocument(data, factory);
    NodeFactory.set(doc, factory);
    return doc;
  }

  parseFromFile(file, factory = new NodeFactory()) {
    let xml = readFileSync(file);
    return this.parseFromString(xml, file, factory);
  }
}

function GetNode(obj, owner, factory) {
  let type = GetType(obj, owner);

  if(type === Entities.Text && typeof obj == 'string') obj = owner.indexOf(obj);

  factory ??= NodeFactory.for(owner);
...
export class Interface {
  ...
  get childNodes() {
    ...
    return NodeFactory(this).createNodeList((raw.children ??= []), this, NodeList);
  }
  ...
  querySelectorAll(...selectors) {
    ...
    return [...query(node, selectors)].map(p => applyPath(p, this));
  }
}
...
define(Node, {
  ...
  path(arg, path = []) {
    if(arg.ownerElement) {
      let child = { NodeList: ['children'], NamedNodeMap: ['attributes'], Attr: ['attributes', arg.name] }[
...
export class NodeList {
  constructor(obj, owner) {
    ...
    const factory = NodeFactory.for(owner);
...
define(NodeList.prototype, {
  ...
  *[Symbol.iterator]() {
    const raw = Node.raw(this);
    const fac = NodeFactory(this);
    for(let node of raw) yield fac(typeof node == 'string' ? Entities.Text : Entities.Element)(node, this);
  }
});
...
export class Element extends Node {
  ...
  get children() {
    ...
    return NodeFactory.for(this).createNodeList((raw.children ??= []), this);
  }
...
export class Document extends Element {
  ...
  createTreeWalker(
    root,
    whatToShow = TreeWalker.TYPE_ALL,
    filter = { acceptNode: node => TreeWalker.FILTER_ACCEPT },
  ...
  [inspectSymbol](depth, opts) {
    ...
    return `\x1b[1;31mDocument\x1b[0m ${tagName} attributes: ${attributes[inspectSymbol](
      depth + 1,
      opts
    )} children: ${inspect(children.map(c => c[inspectSymbol](depth + 2, opts)))}>`;
  }
}
