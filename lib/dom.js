...
import { wrapFunction, isInstanceOf, types, className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, mapObject, getset, getter, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, isSymbol, isPropertyKey, memoize, modifier, quote, range, properties, isPrototypeOf, weakMapper, decodeHTMLEntities, } from 'util';
...
export class Factory {
  ...
  static get(node) {
    ...
    if(node.nodeType != DOCUMENT_NODE) 
      if((tmp = Node.document(node))) node = tmp;
  ...
  querySelector(s) {
    ...
    try {
      for(let sel of parseSelectors(s)) {
        ...
        DEBUG('querySelector', ...values.map(v => v + ''));
