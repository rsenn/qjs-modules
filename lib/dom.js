...
function applyPath(path, obj) {
  //console.log('applyPath', console.config({ compact: true, breakLength: Infinity }), { path: path.join('.'), obj });
...
export function Factory(types = Prototypes()) {
  ...
  if(new.target) {
    const obj = types;
    const GetProto = type => ((type = isNumber(type) ? EntityNames[type] : type), obj[type] ?? Prototypes()[type]);
    ...
    if(!isFunction(types)) {
      create = (type, ...args) => new (GetProto(type).constructor)(...args);
    }
    ...
    for(let i = 0; i < EntityNames.length; i++) {
      ...
      result.classes ??= Object.create(null);
      result.classes[name] = proto.constructor;
    }
