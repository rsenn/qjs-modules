...
define(
  ...
  nonenumerable({
    ...
    inspect(obj) {
      ...
      for(let i = 0; obj[i]?.name; i++) {
        const part = obj[i];

        DEBUG('NamedNodeMap.inspect', { i, part });

        let s = part[Symbol.inspect]?.(0, {}) ?? '';

        let pos = s.indexOf('Attr');
      ...
      if(isPrototypeOf(Element.prototype, obj[0])) return `\x1b[1;31m${className(obj)}\x1b[0m {\n  ` + a.join(',\n  ') + `\n}`;
