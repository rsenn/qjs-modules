...
export class CharacterData extends Node {
  ...
  appendData(data) {
    const s = charData(this) + data;
    charData(this, s);
    return s;
  }

  deleteData(offset, count) {
    const s = charData(this);
    charData(this, s.slice(0, offset) + s.slice(offset + count));
  }

  insertData(offset, data) {
    const s = charData(this);
    charData(this, s.slice(0, offset) + data + s.slice(offset));
  }

  replaceData(offset, count, data) {
    const s = charData(this);
    charData(this, s.slice(0, offset) + data + s.slice(offset + count));
  }
}
