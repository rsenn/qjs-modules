...
import { extend, arrayFacade, assert, camelize, decamelize, define, getset, gettersetter, isObject, isFunction, lazyProperties, memoize, modifier, quote, range, properties } from 'util';
...
function GetNode(obj, owner, factory) {
  ...
  if(type === Entities.Text && typeof obj == 'string' && isFunction(owner.indexOf)) obj = owner.indexOf(obj);
...
export const NODE_TYPES = /*properties*/ {
  ATTRIBUTE_NODE,
  CDATA_SECTION_NODE,
  COMMENT_NODE,
  DOCUMENT_FRAGMENT_NODE,
...
define(Node.prototype, { [Symbol.toStringTag]: 'Node' });
define(Node, {
  raw(node) {
    checkNode(node);
