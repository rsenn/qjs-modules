...
export class Interface {
  ...
  *querySelectorAll(...selectors) {
    ...
    for(let sel of parseSelectors(...selectors)) {
      console.log('*querySelectorAll', console.config({ compact: true }), { sel: sel +'' });

      for(let p of select(raw, sel ?? (() => true), RETURN_PATH, TYPE_OBJECT, ['children'])) {
        console.log('*querySelectorAll', console.config({ compact: true }), { p });
        const r = applyPath(p, this);

        //for(let r of queryAll(raw, [sel], p => applyPath(p, this))) {
        //console.log('*querySelectorAll', console.config({ compact: true }), r);

        if(r) {
          if(isObject(r) && Symbol.iterator in r) yield* r;
          else yield r;
        }
      }
    }
