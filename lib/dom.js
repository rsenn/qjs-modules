...
function setParentOwner(node, ancestor) {
  ...
  //console.log('setParentOwner', console.config({ compact: true }), { ok, node, ancestor });
...
export class MutationRecord {
  ...
  static attribute(name, ns = null, target, oldValue) {
    const r = new MutationRecord();
    define(r, { type: 'attribute', attributeName: name, attributeNamespace: ns, target, oldValue });
    return r;
  }

  static characterData(target, oldValue) {
    const r = new MutationRecord();
    define(r, { type: 'attribute', target, oldValue });
    return r;
  }

  static childList(target) {
    const r = new MutationRecord();
    define(r, { type: 'childList', taregt, oldValue: null });
    return r;
  }
}
...
export class MutationObserver {
  ...
  observe(target, options = {}) {
    let { subtree = false, childList = false, attributes, attributeFilter, attributeOldValue, characterData, characterDataOldValue = false };

    attributes ??= isObject(attributeFilter) || attributeOldValue !== undefined;
    characterData ??= characterData !== undefined;

    MutationObserver.#observe.set(target, options);
    this.#targets.add(target);
  }

  disconnect() {
    for(const target of this.#targets) this.#observe.delete(target);
  }
}
