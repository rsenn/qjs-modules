...
import { className, nonenumerable, extend, arrayFacade, assert, camelize, decamelize, define, mapObject, getset, gettersetter, isBool, isObject, isFunction, isNumeric, isNumber, isString, lazyProperties, memoize, modifier, quote, range, properties, } from 'util';
  ...
  constructor(obj, owner, pred = e => true) {
    ...
    const coll = new Proxy(this, {
      get: (target, prop, receiver) => {
        ...
        if(isIndex(prop) && prop >= 0 && prop < arr().length) return GetNode(arr()[prop], coll, Factory.for(owner));

        return Reflect.get(target, prop, receiver);
      },
      getOwnPropertyDescriptor: (target, prop) => {
        if(prop == 'length' || (prop >= 0 && prop < arr().length)) return { value: arr()[prop], configurable: true, enumerable: true };
...
export function NamedNodeMap(delegate, owner) {
  ...
  const getset = isFunction(delegate) ? delegate : gettersetter(delegate);
  const adapter = mapObject(getset);

  const isAttr = prop => isString(prop) && adapter.has(prop);
  const wrapAttr = (value, prop) => new Attr([(...args) => getset(prop, ...args), prop], obj);
  ...
  const obj = new Proxy(this, {
    get: (target, prop, receiver) => {
      ...
      if(isString(prop)) {
        ...
        if(adapter.has(prop)) return wrapAttr(adapter.get(prop), prop);
      }
      return Reflect.get(target, prop, receiver);
    },
  ...
  NamedNodeMap.prototype,
  {
    ...
    *[Symbol.iterator]() {
    ...
    [Symbol.inspect]() {
      const raw = Node.raw(this);
      let str = '';
      for(const attr in raw) str += ' \x1b[1;35m' + attr + '\x1b[1;36m="' + raw[attr] + '"\x1b[0m';
      return '{' + str + ' }';
    },
  },
  { enumerable: false },
...
export class Element extends Node {
  ...
  get attributes() {
    return Factory.for(this).NamedNodeMap.cache((Node.raw(this).attributes ??= {}), this);
  }
  ...
  [Symbol.inspect](depth, opts) {
    ...
    str += '>';
    try {
      const tmp = children.length > 0 ? `  *** ${children.length} children ***  ` : '';
  ...
  static xpath(elem, attr = 'name') {
    ...
    for(let e of Node.hier(elem, n => n.nodeType == ELEMENT_NODE, false)) {
      let s = e.tagName,
...
extend(Element.prototype, { [Symbol.toStringTag]: 'Element', nodeType: ELEMENT_NODE, namespaceURI: 'http://www.w3.org/1999/xhtml' }, { enumerable: false });
...
export function ListAdapter(list, getKey = 'name') {
  if(!isFunction(getKey)) {
    const key = getKey;
    getKey = item => Node.raw(item).attributes[key];
  }

  return Object.setPrototypeOf(
    {
      get: key => FindItem(list, item => getKey(item) == key),
      keys: () => [...MapItems(list, (item, index, list) => getKey(item))],
      has(key) {
...
extend(ListAdapter.prototype, { [Symbol.toStringTag]: 'ListAdapter' }, { enumerable: false });
