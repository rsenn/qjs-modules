...
const EntityNames = ['Document', 'Node', 'NodeList', 'Element', 'NamedNodeMap', 'Attr', 'Text', 'Comment', 'TokenList', 'CSSStyleDeclaration', 'HTMLCollection', 'Event', 'EventTarget', 'DocumentFragment', 'HTMLElement'];
...
export class DOMStringMap {
  ...
  [Symbol.iterator]() {
    ...
    return Object.keys(Node.raw(element).attributes ?? {})
      ...
      .map(k => [camelize(k.slice(5)), element.getAttribute(k)])[Symbol.iterator]();
  }
...
const datasetHandler = {
  ...
  getOwnPropertyDescriptor(map, name) {
    if(datasetHandler.has(map, name))
      return { configurable: true, enumerable: true, value: datasetHandler.get(map, name) };
  },
};
...
extend(HTMLElement.prototype, nonenumerable({
...
export class HTMLFormElement extends HTMLElement {
  ...
  get elements() {
    ...
    for(const child of this.children) {
      if(child.nodeType === ELEMENT_NODE && ['input', 'select', 'textarea', 'button'].includes(child.tagName))
        controls.push(child);
    }
    return controls;
  }
...
extend(DocumentFragment.prototype, nonenumerable({
