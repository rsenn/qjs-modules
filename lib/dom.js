  ...
  path(arg, path = []) {
    if(arg.ownerElement)
      return Node.path(arg.ownerElement).concat(
        { NodeList: ['children'], NamedNodeMap: ['attributes'], Attr: ['attributes', arg.name] }[
  ...
  get lastElementChild() {
    ...
    if(children?.length)
      for(let i = children.length - 1; i >= 0; i--)
        if(isObject(children[i]) && 'tagName' in children[i])
          return Element.cache(children[i], this.children);
  }
...
export class Text extends Node {
  ...
  [inspectSymbol](depth, opts) {
    const { data } = this;
    return `\x1b[1;31m${this[Symbol.toStringTag]}\x1b[0m \x1b[38;2;192;2550m${quote(
      data,
      "'"
    )}\x1b[0m`;
  }
