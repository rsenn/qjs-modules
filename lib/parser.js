...
import { extend, className } from 'util';
...
export class Rule {
  /* Backtracking on failure also has to undo any Capture nodes a sub-rule
   * pushed before the overall attempt failed - e.g. Alternative trying
   * "cmd_name cmd_suffix" then falling back to "cmd_name" alone must not
   * leave the first attempt's (successful, then abandoned) cmd_name node in
   * the tree alongside the second attempt's own. Truncating the current
   * top astStack frame back to its pre-attempt length does this generically
   * for every combinator that goes through Rule.match, without Capture
   * itself needing to know anything about the outcome of whatever's above it. */
  static match(lexer, fn = lex => false) {
    const pos = lexer.loc.clone();
    const frame = lexer.astStack?.[lexer.astStack.length - 1];
  ...
  toSource() {
    ...
    throw new Error(`Unimplemented 'toSource' on class '${className(this)}'`);
  }
}
 ...
 * doc/js/predicate.md for how this dispatch
 * works in general. */
...
export class Sequence extends Rule {
  ...
  toSource() {
    return '(' + this.rules.map(r => r.toSource()).join(' >> ') + ')';
  }
}
...
export class Expect extends Rule {
  ...
  toSource() {
    return this.left.toSource() + ' << ' + this.right.toSource();
  }
}
...
/**
 * boost::spirit's `a | b` (alternative): tries `left` first, then `right`
 * only if `left` failed - first successful branch wins, no attempt at
 * longest-match. Each branch already backtracks itself via its own
 * `.match()`, so a failed `left` leaves the lexer exactly where `right`
 * needs to start trying. Built by the `|` operator below; like `Sequence`,
 * `a | b | c` flattens into one Alternative of three branches rather than
 * nesting pairs.
 */
export class Alternative extends Rule {
  constructor(...rules) {
    super();
    this.rules = rules;
  }
  ...
  match(lexer) {
    return Rule.match(lexer, lex => this.rules.some(rule => (typeof rule == 'function' ? rule(lex) !== false : rule.match(lex))));
  }
...
extend(Alternative.prototype, { [Symbol.toStringTag]: 'Alternative' });
...
/**
 * boost::spirit's `lit("...")`: matches a whole literal string/token
 * sequence as one unit (rather than `_char()`-per-character via a
 * `Sequence`), which also makes it a single backtracking step instead of
 * one per character.
 */
export class Literal extends Rule {
  ...
  match(lexer) {
    return Rule.match(lexer, lex => {
      for(const item of this.value) if(lex.next() !== item) return false;
...
extend(Literal.prototype, { [Symbol.toStringTag]: 'Literal' });

export function _lit(value) {
  return new Literal(value);
}

/**
 * boost::spirit's `char_(from, to)` / a `[...]`-style character class:
 * matches one element for which `test` returns true. `_charRange()` and
 * `_charSet()` below build the common cases; `_charPred()` (or `new
 * CharClass(test)` directly) accepts an arbitrary predicate for anything
 * else (negation, unions of ranges and singles, ...).
 */
export class CharClass extends Rule {
  #source;
  ...
  match(lexer) {
    return Rule.match(lexer, lex => {
      const item = lex.next();
      return item !== undefined && this.test(item);
    });
  }
...
extend(CharClass.prototype, { [Symbol.toStringTag]: 'CharClass' });

export function _charPred(test) {
  return new CharClass(test);
}

export function _charRange(lo, hi) {
  return new CharClass(c => c >= lo && c <= hi, '[' + lo + '-' + hi + ']');
}

export function _charSet(chars) {
  const set = new Set(chars);
  return new CharClass(c => set.has(c), '[' + [...set].join('') + ']');
}

/**
 * Records a labeled parse-tree node - `{ name, start, end, children }`,
 * `start`/`end` being whatever position values the lexer's `.charPos`
 * uses (character offsets for stringInput(), token indices for a token
 * stream, ...) - whenever the wrapped `rule` matches. Not a boost::spirit
 * primitive by this name; the closest equivalent there is a `rule<Iterator,
 * Ast()>` with a synthesized attribute. Deliberately keeps `Rule.match()`'s
 * plain-boolean signature everywhere else unchanged: tree-building state
 * lives in a stack attached to the lexer object itself (`lexer.astStack`,
 * created lazily), rather than threading an extra parameter through every
 * combinator's `match(lexer)`.
 *
 * Each match pushes a fresh "children" frame before running `rule`, so
 * nested Captures collect into the right parent: on success, the finished
 * node is appended to the *now-current* (parent) frame and `true` is
 * returned; on failure, the frame is simply discarded (matching every other
 * combinator's silent-backtrack-on-failure) and `false` is returned - a
 * failed sub-parse never pollutes the tree.
 *
 * After a full parse, `captureRoot(lexer)` (below) retrieves whatever ended
 * up in the outermost frame.
 */
export class Capture extends Rule {
  ...
  match(lexer) {
    const stack = (lexer.astStack ??= [[]]);
    const start = lexer.charPos;

    stack.push([]);

    const ok = typeof this.rule == 'function' ? this.rule(lexer) !== false : this.rule.match(lexer);

    const children = stack.pop();

    if(!ok) return false;

    stack[stack.length - 1].push({ name: this.label, start, end: lexer.charPos, children });

    return true;
  }
}

extend(Capture.prototype, { [Symbol.toStringTag]: 'Capture' });

export function _capture(name, rule) {
  return new Capture(name, rule);
}

/* The node(s) collected in the outermost Capture frame after a top-level
 * `.match()` call - i.e. the parse tree, root-first. Empty if nothing built
 * by a Capture ever matched (e.g. the grammar has no Capture at all, or the
 * top-level match failed before reaching one). */
export function captureRoot(lexer) {
  return lexer.astStack?.[0] ?? [];
}
 ...
 * doc/js/predicate.md step 5) - harmless for Number (a no-op), but
...
/* boost::spirit's `|` alternative operator: `a | b` tries `a`, then `b` if
 * `a` failed (see Alternative above). Flattens chains the same way
 * `sequence()` does. */
function alternative(a, b) {
  const flatten = x => {
    if(x instanceof Alternative) return x.rules;
    if(typeof x == 'function' || x instanceof Rule) return [x];
    return [new Rule(x)];
  };

  return new Alternative(...flatten(a), ...flatten(b));
}
 ...
 * see doc/js/predicate.md for why that's what makes Rule's own left/right
