...
export class Parser {
  constructor(lexer) {
    ...
    this.processed = [];
    this.tokens = lexer
      ? lexer.tokens.reduce((acc, name, id) => ((acc[name] = id), acc), {})
      : null;
  }
  ...
  expect(tokens) {
    ...
    if(!ret)
      throw new Error(`${tok.loc} Expecting ${tokens
          .map(tok => (typeof tok == 'number' ? this.lexer.tokens[tok] : tok))
          .join('|')}, got ${tok.type} '${tok.lexeme}'`
      );
    return tok;
  }
}
