...
export class BNFLexer extends Lexer {
  ...
  constructor(input, filename) {
    ...
    this.handler = (arg, tok) =>
      console.log('Unmatched token at ' +
          arg.loc +
          ' state=' +
  ...
  addRules() {
    ...
    0;
    this.addRule('cstart', /%{/, () => this.pushState('C'));
    this.addRule('action', /{/, () => this.pushState('C'));
    ...
    // this.addRule('l_expr', /<LEXDEFINE>[^\n]*/, () => this.popState());

    this.addRule('p_section', /<LEXPATTERN>%%\n/);
    this.addRule('p_literal', /<LEXPATTERN>\"{DoubleStringCharacter}*\"/);
    this.addRule('p_state', /<LEXPATTERN><\w+>/);
    this.addRule('p_lp', /<LEXPATTERN>\(/);
    this.addRule('p_rp', /<LEXPATTERN>\)/);
    this.addRule('p_bar', /<LEXPATTERN>\|/);
    this.addRule('p_subst', /<LEXPATTERN>{\w+}/);
    this.addRule('p_dot', /<LEXPATTERN>\./);
    this.addRule('p_escape', /<LEXPATTERN>\\./);
    this.addRule('p_postfix', /<LEXPATTERN>[?*+]/);
    this.addRule('p_cstart', /<LEXPATTERN>[ \t]+{/ /*, () => this.pushState('C')*/);
    this.addRule('p_class', /<LEXPATTERN>\[([^\]\\]|\\.)+\]/);
    //this.addRule('p_pattern', /<LEXPATTERN>{RegularExpressionBody}/);
    this.addRule('p_newline', /<LEXPATTERN>\r?\n/);
    this.addRule('p_char', /<LEXPATTERN>./);
    this.addRule('l_pattern2', /<LEXRULES>[^\s]+/, () => this.push('LEXACTION'));
    this.addRule('l_newline2', /<LEXACTION>\r?\n/, (lexer, skip) => (skip(), lexer.popState()));
    this.addRule('l_end', /<LEXRULES>\/lex/, () => this.popState());
