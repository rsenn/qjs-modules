...
export class BNFLexer extends Lexer {
  ...
  addRules() {
    ...
    this.addRule('<directive>string', /"(\\.|[^"\n])*"/, DIRECTIVE);
    this.addRule('<directive>newline', /\r?\n/, DIRECTIVE, () => BNF);
    this.addRule('<directive>identifier', /[.A-Za-z_][-\w]*/, DIRECTIVE);
    this.addRule('<directive>number', /[0-9]+/, DIRECTIVE);
    this.addRule('<directive>name', /<([-\w]*)>/, DIRECTIVE);
    this.addRule('any', /<>/, DIRECTIVE | BNF);
    ...
    for(let name in CTokens) {
     ...
     console.log('c_' + name, Lexer.toString(CTokens[name]), C, fn);
      this.addRule('c_' + name, CTokens[name], C, fn);
    }
    ...
    for(let [name, expr, mask = JS] of JSRules) {
      ...
      this.addRule('js_' + name, expr, mask, fn);
    }
