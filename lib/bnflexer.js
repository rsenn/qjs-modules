...
export class BNFLexer extends Lexer {
  ...
  addRules() {
    ...
    this.addRule('d_string', /"(\\.|[^"\n])*"/, DIRECTIVE);
    this.addRule('d_newline', /\r?\n/, DIRECTIVE, () => BNF);
    this.addRule('d_identifier', /[.A-Za-z_][-\w]*/, DIRECTIVE);
    this.addRule('d_number', /[0-9]+/, DIRECTIVE);
    this.addRule('d_name', /<([-\w]*)>/, DIRECTIVE);
