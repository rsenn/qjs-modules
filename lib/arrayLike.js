export class ArrayLike {
  ...
  forEach(callbackFn, thisArg) {
    const { length } = this;
    for(let i = 0; i < length; ++i) callbackFn.call(thisArg, this[i], i, this);
  }

  *map(callbackFn, thisArg) {
    const r = [],
      n = this.length;
    for(let i = 0; i < n; ++i) r.push(callbackFn.call(thisArg, this[i], i, this));
    return r;
  }
  ...
  reduce(callbackFn, initialValue) {
    const { length } = this;
    let o = initialValue;
    for(let i = 0; i < length; ++i) o = callbackFn.call(thisArg, o, this[i], i, this);
    return o;
  }
}
