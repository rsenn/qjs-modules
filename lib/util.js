...
import { quote, escape, isObject, memoize, inherits, getset, modifier, getter, setter, gettersetter, hasGetSet, mapObject, once, waitFor, define, weakAssign, getPrototypeChain, getConstructorChain, hasPrototype, filter, curry, split, unique, getFunctionArguments } from './misc.js';
export {
  quote,
  escape,
  isObject,
...
export const inspect = process.release.name == 'quickjs' ? requireModule('inspect') : require('util').inspect;
const misc = process.release.name == 'quickjs' ? requireModule('misc') : require('./misc.js');

const timers = process.release.name == 'quickjs' ? requireModule('os') : require('timers');
const TextDecoder =
  process.release.name == 'quickjs' ? requireModule('textcode').TextDecoder : require('string_decoder').StringDecoder;
...
export const isFunction = misc.isFunction;*/
...
export function formatWithOptions(opts, ...args) {
  if(!isObject(opts)) throw new TypeError(`options argument is misc.not an object`);
