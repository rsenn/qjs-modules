...
import { puts, exit, strerror } from 'std';
...
export function addremovehas(target, ...args) {
  ...
  if(isObject(target)) {
    if(target instanceof Set || ['add', 'delete', 'has'].every(n => isFunction(target[n])))
      return [
        el => (target.add(el), target),
        el => (target.delete(el), target),
        el => (target.has(el), target)
      ];
...
export function push(obj, ...values) {
  ...
  if(isFunction(obj.push)) {
    ...
    (obj.length === 0 || obj[obj.length - 1] !== undefined)
  ) {
...
export function mapFunction(map) {
  ...
  if(map instanceof Map || (isObject(map) && isFunction(map.get) && isFunction(map.set))) {
    ...
    map instanceof Cache ||
    (isObject(map) && isFunction(map.match) && isFunction(map.put))
  ) {
...
export const getSystemErrorMap = once(
  () =>
    new Map(
      errors.reduce((acc, name, i) => (i > 0 && acc.push([-i, [name, strerror(i)]]), acc), [])
    )
);
