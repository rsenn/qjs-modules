...
export const isPrototypeOf = (proto, obj) => Object.prototype.isPrototypeOf.call(proto, obj);
...
export function hasOwnProperty(o, m) {
...
export function getter(target, ...args) {
  ...
  if((r = (getter.map ??= weakMapper())(target))) return r;
...
export function setter(target, ...args) {
  ...
  if((r = (setter.map ??= weakMapper())(target))) return r;
...
export function gettersetter(target, ...extraArgs) {
  ...
  if(Array.isArray(target)) {
    ...
    if(!('keys' in target) && target.get === target.set) fn = (...args) => target.set(...args);
    else if('keys' in target) fn = (...args) => (args.length == 0 ? [...target.keys()] : args.length <= (target.get.length ?? 1) ? target.get(...args) : target.set(...args));
    else fn = (...args) => (args.length <= (target.get.length ?? 1) ? target.get(...args) : target.set(...args));
  } else if(isObject(target)) {
    fn = (...args) => {
...
export function mapObject(target) {
  ...
  if(hasGetSet(target)) obj = target;
    ...
    obj = { get: target, set: target };
    try {
      if(Array.isArray(target())) obj.keys = target;
    } catch(e) {}
    if('keys' in obj) obj.has = key => [...obj.keys()].indexOf(key) != -1;
  } else if(isObject(target))
    obj = {
  ...
  if(obj !== target) define(obj, nonenumerable({ receiver: target }));
...
export function weakDefine(obj, ...args) {
  ...
  for(let other of args) {
    const otherDesc = Object.getOwnPropertyDescriptors(other);
...
export function merge(...args) {
  ...
  if(isMap) {
    r = new Map();
    for(let arg of args) for (let [key, value] of entries(arg)) r.set(key, value);
  } else {
...
export function getConstructorChain(obj, ...range) {
  ...
  for(let proto of getPrototypeChain(obj, ...range)) pushUnique(r, proto.constructor);
  return r;
}
...
export function className(obj) {
  if(isObject(obj)) {
    ...
    /*for(let p of getPrototypeChain(obj)) {
      if(!isObject(p)) break;
      if(hasOwnProperty(p, Symbol.toStringTag)) return p[Symbol.toStringTag];
      if(hasOwnProperty(p, 'constructor')) return functionName(p.constructor);
    }*/
