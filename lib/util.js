...
import { atob, btoa, bits, dupArrayBuffer, enqueueJob, escape, getpid, getPrototypeChain, getTypeName, isArray, isArrayBuffer, isBigInt, isBool, isConstructor, isFunction, isInteger, isNumber, isObject as isObject2, isString, isSymbol, rand, searchArrayBuffer, toString, } from 'misc';
export { atob, btoa };
import { exit, puts, strerror } from 'std';
export { setInterval, clearInterval } from 'timers';
...
export const noop = () => {};
export const isNative = fn => /\[native\scode\]/.test(stringify(fn));
export const isPropertyKey = k => isString(k) || isSymbol(k);
export const isObject = o => (typeof o == 'object' && o !== null) || typeof o == 'function';
export const isClass = o => isConstructor(o) && /^class\b/.test(o + '');
export const typeIsObject = isObject2;
...
export const GeneratorPrototype = getPrototypeOf(getPrototypeOf((function* () {})()));
export const AsyncGeneratorPrototype = getPrototypeOf(getPrototypeOf((async function* () {})()));
...
export const types = {
  ...
  isAsyncFunction: o => isInstanceOf(AsyncFunction, o) || o == AsyncGeneratorPrototype.next,
  isGenerator: o => isPrototypeOf(GeneratorPrototype, o),
  isGeneratorFunction: o => isInstanceOf(GeneratorFunction, o),
  isAsyncGenerator: o => isPrototypeOf(AsyncGeneratorPrototype, o),
...
export function getter(target, ...args) {
  ...
  if(Array.isArray(target) && target.every(isFunction)) {
    r = target[0];
...
export function properties(obj, options = { enumerable: true }) {
  ...
  for(const prop of keys(obj)) {
    if(Array.isArray(obj[prop])) {
      const [get, set, opts2 = {}] = obj[prop];
      desc[prop] = { ...opts, ...opts2, get, set };
      ...
      desc[prop] = memo
        ? {
            ...
            configurable: true,
            get() {
              const value = obj[prop].call(this);
              Object.defineProperty(this, prop, { ...opts, configurable: true, value });
              return value;
            },
          }
        : { ...opts, get: obj[prop] };
...
/**
 * Whether the calling module is the entry script (run directly), not merely
 * imported by one. `import.meta` is per-module, so the caller must pass its own
 * `import.meta.url` - this can't read it on the caller's behalf: `import.meta.url`
 * lexically refers to whichever file the expression appears in.
 *
 * @example
 *   if (isMainModule(import.meta.url)) main(...scriptArgs.slice(1));
 */
export function isMainModule(moduleUrl) {
  const [real] = os.realpath(scriptArgs[0]);
  return !!real && moduleUrl == `file://${real}`;
}
...
export function catchFunction(fn, onReject) {
  const result = [fn, onReject].some(isAsync)
    ? async function(...args) {
        let r;
        try {
          r = await fn.apply(this, args);
        } catch(err) {
          return await onReject(err, ...args);
        }
        return r;
      }
    : function(...args) {
        ...
        try {
          ...
          return onReject(err, ...args);
        }
        return r;
      };

  return copyFunctionNameAndLength(result, fn);
}

export function finallyFunction(fn, onFinally) {
  const result = [fn, onFinally].some(isAsync)
    ? async function(...args) {
        let r;
        try {
          r = await fn.apply(this, args);
...
export function tryFunction(fn, onResolve, onReject) {
  onResolve ??= a => a;

  const is_async = [fn, onResolve, onReject].some(isAsync);
  const result = onReject
    ? is_async
      ...
      : function(...args) {
          ...
          return onResolve(r, ...args);
        }
    : is_async
      ? async function(...args) {
          return await onResolve(await fn.apply(this, args));
        }
      : function(...args) {
          return onResolve(fn.apply(this, args));
        };
...
export function mapFunction(map) {
  ...
  if(map instanceof Map || (isObject(map) && isFunction(map.get) && isFunction(map.set))) {
    declare(fn, { set: (key, value) => (map.set(key, value), undefined), get: key => map.get(key) });
    /*} else if(map instanceof Cache || (isObject(map) && isFunction(map.match) && isFunction(map.put))) {
    declare(fn, { set: (key, value) => (map.put(key, value), undefined), get: key => map.match(key) });*/
  } else if(isObject(map) && isFunction(map.getItem) && isFunction(map.setItem)) {
    declare(fn, { set: (key, value) => (map.setItem(key, value), undefined), get: key => map.getItem(key) });
  } else {
    declare(fn, { set: (key, value) => ((map[key] = value), undefined), get: key => map[key] });
  }
  ...
  if(isFunction(fn.entries)) {
    declare(fn, {
      ...
      forEach(pred) {
        let i = 0;
        for(const [key, value] of this.entries()) pred([key, value], i++);
      },
    });
  }

  if(isFunction(map.delete)) declare(fn, { delete: key => map.delete(key) });
  if(isFunction(map.has)) declare(fn, { has: key => map.has(key) });
...
export function weakMapper(createFn = (obj, arg) => arg, map = new WeakMap(), hitFn) {
  define(mapper, { set: (k, v) => map.set(k, v), get: k => map.get(k) });

  if('has' in map) define(mapper, { has: k => map.has(k) });
  if('keys' in map) define(mapper, { keys: k => map.keys(k) });
  if('delete' in map) define(mapper, { delete: k => map.delete(k) });

  define(mapper, { map });

  return mapper;

  function mapper(obj, ...args) {
    let r;
    if(map.has(obj)) {
      r = map.get(obj);
