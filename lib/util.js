...
import { dupArrayBuffer, escape, isArray, isBigDecimal, isBigFloat, isBigInt, isBool, isCFunction, isConstructor, isEmptyString, isError, isException, isExtensible, isFunction, isHTMLDDA, isInstanceOf, isInteger, isJobPending, isLiveObject, isNull, isNumber, isObject, isRegisteredClass, isString, isSymbol, isUncatchableError, isUndefined, isUninitialized, isArrayBuffer, rand, toArrayBuffer, toString, bits, getTypeName, atexit } from 'misc';
...
export { getenv as getEnv } from 'std';
...
export const types = {
  ...
  isArrayLike(v) {
    return isObject(v) && isNumber(v.length) && Number.isInteger(v.length);
  }
};
      ...
      if(x[i] == '%') {
        ...
        if(a + 1 !== v.length) {
          switch (c) {
            case 115: // %s
              const y = v[++a];
              if(isNumber(y)) t = formatNumber(y);
              else if(types.isBigInt(y)) t = `${y}n`;
              else if(!isObject(y) || y === null || !hasBuiltIn(y, 'toString')) t = String(y);
            ...
            case 100: // %d
              const n = v[++a];
              if(types.isBigInt(n)) t = `${n}n`;
              else if(isSymbol(n)) t = 'NaN';
            ...
            case 105: // %i
              const k = v[++a];
              if(types.isBigInt(k)) t = `${k}`;
              else if(isSymbol(k)) t = 'NaN';
...
export function getOpt(options = {}, args) {
  ...
  if(isString(params)) params = params.split(',');
...
export function push(obj, ...values) {
  ...
  } else if(isNumber(obj.length) && (obj.length === 0 || obj[obj.length - 1] !== undefined)) {
...
export function instrument(
  ...
  const isAsync = types.isAsyncFunction(fn) || types.isAsyncFunction(now);
  const doLog = isAsync
    ? async (args, ret) => {
        let t = Date.now();
...
export function mapAdapter(fn) {
  ...
  if(fn[Symbol.iterator]) r.entries = fn[Symbol.iterator];
    ...
    let g = fn();
    if(types.isIterable(g) || types.isGenerator(g)) r.entries = () => fn();
  }
