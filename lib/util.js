...
import { JS_EVAL_FLAG_BACKTRACE_BARRIER, Location, dupArrayBuffer, escape, getPrototypeChain, isArray, isBigDecimal, isBigFloat, isBigInt, isBool, isCFunction, isConstructor, isEmptyString, isError, isException, isExtensible, isFunction, isHTMLDDA, isInstanceOf, isInteger, isJobPending, isLiveObject, isNull, isNumber, isObject, isRegisteredClass, isString, isSymbol, isUncatchableError, isUndefined, isUninitialized, isArrayBuffer, rand, toArrayBuffer, toString, watch } from 'misc';
...
export function getset(target) {
  let ret = [];
  if(typeof target == 'function') ret = [target, target];
  ...
  } else if(isObject(target)) ret = [key => target[key], (key, value) => (target[key] = value)];
  return ret;
}

export function gettersetter(target) {
  let gettersetter;
  if(isObject(target) && typeof target.receiver == 'function') return target.receiver;
  if(typeof target == 'function') gettersetter = target;
  else if(hasGetSet(target)) {
    if(target.get === target.set) gettersetter = (...args) => target.set(...args);
    else gettersetter = (...args) => (args.length == 1 ? target.get(args[0]) : target.set(...args));
  ...
  if(gettersetter !== target) define(gettersetter, { receiver: target });
  return gettersetter;
}
...
export default util;
export { JS_EVAL_FLAG_BACKTRACE_BARRIER, Location, dupArrayBuffer, escape, getPrototypeChain, isArray, isBigDecimal, isBigFloat, isBigInt, isBool, isCFunction, isConstructor, isEmptyString, isError, isException, isExtensible, isFunction, isHTMLDDA, isInstanceOf, isInteger, isJobPending, isLiveObject, isNull, isNumber, isObject, isRegisteredClass, isString, isSymbol, isUncatchableError, isUndefined, isUninitialized, isArrayBuffer, rand, toArrayBuffer, toString, watch } from 'misc';
