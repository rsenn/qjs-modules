...
export function memoize(fn, cache = {}) {
  let memo;
  if(cache instanceof Map || cache instanceof WeakMap)
    memo = function MemoizeMap(n, ...rest) {
      let r;
    ...
    memo = function MemoizeHash(n, ...rest) {
      if(n in cache) return cache[n];
      return (cache[n] = fn.call(this, n, ...rest));
    };
  memo.cache = cache;
  return memo;
}
...
export function getset(obj) {
  let fn = (...args) => (args.length == 1 ? obj.get(...args) : (obj.set(...args), obj));
  fn.target = obj;
  return fn;
}
