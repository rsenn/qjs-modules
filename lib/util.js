...
import { arrayToBitfield, atob, atomToString, atomToValue, bitfieldToArray, btoa, compileScript, concatArrayBuffer, dupArrayBuffer, evalBinary, getByteCode, getClassAtom, getClassConstructor, getClassCount, getClassID, getClassName, getClassProto, getCommandLine, getCurrentWorkingDirectory, getExecutable, getFileDescriptor, getOpCodes, getPerformanceCounter, getProcMaps, getProcMounts, getProcStat, getPrototypeChain, getRootDirectory, getegid, geteuid, getgid, getpid, getppid, getsid, getuid, hrtime, readObject, resizeArrayBuffer, setegid, seteuid, setgid, setuid, toArrayBuffer, toPointer, toString, uname, valuePtr, valueTag, valueToAtom, valueType, writeObject, rand, randi, randf, srand, escape, quote } from 'misc';
import { extendArray, ArrayExtensions } from './extendArray.js';
import { extendGenerator, GeneratorExtensions, GeneratorPrototype, GeneratorConstructor } from './extendGenerator.js';
import * as os from 'os';

const slice = (x, s, e) => (typeof x == 'object' ? (Array.isArray(x) ? Array.prototype.slice.call(x, s, e) : x.slice(s, e)) : String.prototype.slice.call(x, s, e));
const stringify = v => `${v}`;
  ...
  if(typeof x === 'string') {
    ...
    for(let i = 0; i < x.length - 1; i++) {
      if(x[i] == '%') {
        ...
        //console.log('p', p, `'${dupArrayBuffer(x, p, i)}'`);
        if(p < i) s += dupArrayBuffer(x, p, i);
        p = i + 1;
        //console.log('i', i);
        ...
        if(a + 1 !== v.length) {
          switch (c) {
            ...
            case 99: // %c
              ...
              break;
            case 37: // %%
              s += dupArrayBuffer(x, p, i);
              p = i + 1;
              continue;
            default:
              continue;
          }
          if(p !== i - 1) s += dupArrayBuffer(x, p, i - 1);
          let pad = parseInt(f);
    ...
    if(p !== 0) {
      a++;
      j = ' ';
      if(p < x.length) s += dupArrayBuffer(x, p);
    }
  }
  while(a < v.length) {
    const y = v[a];
...
export { arrayToBitfield, atob, atomToString, atomToValue, bitfieldToArray, btoa, compileScript, concatArrayBuffer, dupArrayBuffer, evalBinary, getByteCode, getClassAtom, getClassConstructor, getClassCount, getClassID, getClassName, getClassProto, getCommandLine, getCurrentWorkingDirectory, getExecutable, getFileDescriptor, getOpCodes, getPerformanceCounter, getProcMaps, getProcMounts, getProcStat, getPrototypeChain, getRootDirectory, getegid, geteuid, getgid, getpid, getppid, getsid, getuid, hrtime, readObject, resizeArrayBuffer, searchArrayBuffer, memcpy, setegid, seteuid, setgid, setuid, toArrayBuffer, toPointer, toString, uname, valuePtr, valueTag, valueToAtom, valueType, writeObject, escape, quote } from 'misc';
