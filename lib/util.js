...
export function copyFunctionNameAndLength(result, fn) {
  if(typeof fn.name == 'string' && fn.name != '') (delete result.name, define(result, { name: fn.name }));
  if(typeof fn.length == 'number') (delete result.length, define(result, { length: fn.length }));
  return result;
}
...
export function tryFunction(fn, onResolve, onReject) {
  ...
  const result = onReject
    ? [fn, onResolve, onReject].some(isAsync)
      ? async function(...args) {
          let r;
          try {
            r = await fn.apply(this, args);
