...
export const inspect = process.release.name == 'quickjs' ? require('inspect') : require('util').inspect;
const misc = process.release.name == 'quickjs' ? require('misc') : require('./misc.js');

const timers = process.release.name == 'quickjs' ? require('os') : require('timers');
const TextDecoder =
  process.release.name == 'quickjs' ? require('textcode').TextDecoder : require('string_decoder').StringDecoder;
...
const sliceFn = x =>
  ({
    ...
    ArrayBuffer(start, end) {
      ...
      return misc.dupArrayBuffer(this, start, end - start);
    },
...
/*typeof x == 'object'
    ? misc.isArrayBuffer(x)
      ? misc.dupArrayBuffer(x, s, e)
      : Array.isArray(x)
...
export const types = {
  isAnyArrayBuffer(v) {
    return misc.isObject(v) && (v instanceof ArrayBuffer || v instanceof SharedArrayBuffer);
  },
  isArrayBuffer(v) {
    return misc.isObject(v) && v instanceof ArrayBuffer;
  },
  isBigInt64Array(v) {
    return misc.isObject(v) && v instanceof BigInt64Array;
  },
  isBigUint64Array(v) {
    return misc.isObject(v) && v instanceof BigUint64Array;
  },
  isDate(v) {
    return misc.isObject(v) && v instanceof Date;
  },
  isFloat32Array(v) {
    return misc.isObject(v) && v instanceof Float32Array;
  },
  isFloat64Array(v) {
    return misc.isObject(v) && v instanceof Float64Array;
  },
  isInt8Array(v) {
    return misc.isObject(v) && v instanceof Int8Array;
  },
  isInt16Array(v) {
    return misc.isObject(v) && v instanceof Int16Array;
  },
  isInt32Array(v) {
    return misc.isObject(v) && v instanceof Int32Array;
  },
  isMap(v) {
    return misc.isObject(v) && v instanceof Map;
  },
  isPromise(v) {
    return misc.isObject(v) && v instanceof Promise;
  },
  isProxy(v) {
    return misc.isObject(v) && v instanceof Proxy;
  },
  isRegExp(v) {
    return misc.isObject(v) && v instanceof RegExp;
  },
  isSet(v) {
    return misc.isObject(v) && v instanceof Set;
  },
  isSharedArrayBuffer(v) {
    return misc.isObject(v) && v instanceof SharedArrayBuffer;
  },
  isUint8Array(v) {
    return misc.isObject(v) && v instanceof Uint8Array;
  },
  isUint8ClampedArray(v) {
    return misc.isObject(v) && v instanceof Uint8ClampedArray;
  },
  isUint16Array(v) {
    return misc.isObject(v) && v instanceof Uint16Array;
  },
  isUint32Array(v) {
    return misc.isObject(v) && v instanceof Uint32Array;
  },
  isWeakMap(v) {
    return misc.isObject(v) && v instanceof WeakMap;
  },
  isWeakSet(v) {
    return misc.isObject(v) && v instanceof WeakSet;
  },
  isDataView(v) {
    return misc.isObject(v) && v instanceof DataView;
  },
  isBooleanObject(v) {
    return misc.isObject(v) && v instanceof Boolean;
  },
  isAsyncFunction(v) {
    return misc.isObject(v) && v instanceof AsyncFunction;
  },
  isGeneratorFunction(v) {
    return misc.isObject(v) && v instanceof GeneratorFunction;
  },
  isAsyncGeneratorFunction(v) {
    return misc.isObject(v) && v instanceof AsyncGeneratorFunction;
  },
  isNumberObject(v) {
    return misc.isObject(v) && v instanceof Number;
  },
  isBigIntObject(v) {
    return misc.isObject(v) && v instanceof BigInt;
  },
  isSymbolObject(v) {
    return v && v instanceof Symbol;
  },
  isNativeError(v) {
    return misc.isObject(v) && v instanceof Error && isNative(v.constructor);
  },
  isMapIterator(v) {
    return misc.isObject(v) && protoOf(v) == MapIteratorPrototype;
  },
  isSetIterator(v) {
    return misc.isObject(v) && protoOf(v) == SetIteratorPrototype;
  },
  isStringObject(v) {
    return misc.isObject(v) && v instanceof String;
  },
  isArrayBufferView(v) {
    return misc.isObject(v) && ArrayBuffer.isView(v);
  },
  isArgumentsObject(v) {
  ...
  /* isExternal(v) {
    return misc.isObject(v) && v instanceof External;
  },*/

  isBoxedPrimitive(v) {
    return misc.isObject(v) && [Number, String, Boolean, BigInt, Symbol].some(ctor => v instanceof ctor);
  },

  isGeneratorObject(v) {
    return misc.isObject(v) && protoOf(v) == GeneratorPrototype;
  },
  isTypedArray(v) {
    return misc.isObject(v) && v instanceof TypedArray;
  },
  isModuleNamespaceObject(v) {
    return misc.isObject(v) && v[Symbol.toStringTag] == 'Module';
  },
  isConstructor(v) {
    return misc.isFunction(v) && 'prototype' in v;
  },
  isIterable(v) {
    return misc.isObject(v) && typeof v[Symbol.iterator] == 'function';
  },
  isAsyncIterable(v) {
    return misc.isObject(v) && typeof v[Symbol.asyncIterator] == 'function';
  },
  isIterator(v) {
    return misc.isObject(v) && typeof v.next == 'function';
  },
  isArrayLike(v) {
    return misc.isObject(v) && typeof v.length == 'number' && Number.isInteger(v.length);
  }
};
...
export function formatWithOptions(opts, ...args) {
  if(!misc.isObject(opts)) throw new TypeError(`options argument is misc.not an object`);
        ...
        if(a + 1 !== v.length) {
          switch (c) {
            case 115: // %s
              ...
              else if(typeof y !== 'object' || y === null || !hasBuiltIn(y, 'misc.toString')) t = String(y);
...
export function clearInterval(id) {
  ...
  if(obj) timers.clearTimeout(obj.id);
}
...
export function getter(target, ...args) {
  if(misc.isObject(target) && misc.isFunction(target.get)) return () => target.get(...args);
  let ret;
  if(misc.isFunction(target)) {
    ret = target;
  } else if(hasGetSet(target)) {
...
export function setter(target, ...args) {
  if(misc.isObject(target) && misc.isFunction(target.set)) return value => target.set(...args, value);
  let ret;
  if(misc.isFunction(target)) {
    ret = target;
  } else if(hasGetSet(target)) {
...
export function gettersetter(target, ...args) {
  let fn;
  if(misc.isObject(target) && misc.isFunction(target.receiver))
    return (...args2) => target.receiver(...args, ...args2);
  if(misc.isFunction(target)) {
    if(misc.isFunction(args[0]) && args[0] !== target) {
...
export function hasGetSet(obj) {
  return misc.isObject(obj) && ['get', 'set'].every(m => typeof obj[m] == 'function');
}
...
export function filter(seq, pred, thisArg) {
  if(misc.isObject(pred) && pred instanceof RegExp) {
    let re = pred;
...
function getAnsiStyles() {
  ...
  const styles = {
    modifier: {
      reset: [0, 0],
      // 21 isn't widely supported misc.and 22 does the same thing
      bold: [1, 22],
      dim: [2, 22],
      italic: [3, 23],
      underline: [4, 24],
  ...
  // Alias bright black as gray (misc.and grey)
  ...
  Object.defineProperties(styles, {
    rgbToAnsi256: {
      value: (red, green, blue) => {
        ...
        // black misc.and white. normal palette only has 4 greyscale shades.
...
export function symmetricDifference(a, b) {
  let is_set = types.isSet(a) && types.isSet(b);
  if(!is_set) {
  ...
  let ret = [...a].filter(x => !b.has(x)).concat([...b].filter(x => !a.has(x)));
  return is_set ? new Set(ret) : ret;
}

export function* difference(container, excludes) {
  if(!Array.isArray(container)) container = [...container];
  if(!Array.isArray(excludes)) excludes = [...excludes];
  for(let x of container) if(excludes.indexOf(x) == -1) yield x;
}

export function union(...args) {
  const is_set = args.every(types.isSet);
  let ret = new Set();
  for(let arg of args) for (let item of arg) ret.add(item);
  return is_set ? ret : [...ret];
}

export function intersection(...args) {
  const is_set = args.every(types.isSet);
  let set = [...new Set(args.shift())];

  while(args.length) {
    let other = new Set(args.shift());
    set = set.filter(item => other.has(item));
  }
  return is_set ? new Set(set) : set;
}

export const quote = misc.quote;
export const escape = misc.escape;
export const isObject = misc.isObject;
...
util.toString = misc.toString;
util.toArrayBuffer = misc.toArrayBuffer;
...
util.errors = errors;
util.getPrototypeChain = misc.getPrototypeChain;
...
//export {inspect, misc, timers, TextDecoder };
...
//export default util;
/*export {
  misc.JS_EVAL_FLAG_BACKTRACE_BARRIER,
  misc.Location,
  misc.dupArrayBuffer,
  misc.escape,
  misc.getPrototypeChain,
  misc.isArray,
  misc.isBigDecimal,
  misc.isBigFloat,
  misc.isBigInt,
  ...
  misc.isNumber,
  misc.isObject,
  misc.isRegisteredClass,
  misc.isString,
  misc.isSymbol,
  misc.isUncatchableError,
  misc.isUndefined,
  misc.isUninitialized,
  misc.isArrayBuffer,
  misc.rand,
  misc.toArrayBuffer,
  misc.toString,
