...
import { ArrayExtensions } from './extendArray.js';
import { extendArray } from './extendArray.js';
import { AsyncGeneratorExtensions } from './extendAsyncGenerator.js';
import { extendGenerator } from './extendGenerator.js';
import { GeneratorExtensions } from './extendGenerator.js';
import { GeneratorPrototype } from './extendGenerator.js';
...
export function getter(target, ...args) {
  ...
  if(Array.isArray(target)) {
    ...
    if(isFunction(target.keys))
      ret = (...argv) => (argv.length > 0 ? target.get(...args, ...argv) : target.keys(...args));
...
export const getSystemErrorMap = once(
  () => new Map(errors.reduce((acc, name, i) => (i > 0 && acc.push([-i, [name, strerror(i)]]), acc), []))
);
