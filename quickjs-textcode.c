  ...
  switch(enc->encoding) {
...

        {
          ...
          if(!libutf_c32_to_c16(cp, &len, u16))
            return JS_ThrowInternalError(
                ctx, "%s: TextEncoder: not a valid code point at (%zu) [%zu]: %" PRIu32, __func__, i, end - ptr, cp);
    ...
    case UTF32: {
      ...
      for(i = 0; ptr < end; ptr = next, i++) {
        ...
        if(cp == 0xffffffff)
          return JS_ThrowInternalError(
              ctx, "%s: TextEncoder: not a valid code point at (%zu): %" PRIu32, __func__, ptr - in.block.base, cp);
        /*cp = 0;
