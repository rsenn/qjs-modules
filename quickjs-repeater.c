...
js_repeater_new(JSContext* ctx, JSValueConst proto, JSValueConst value) {
  ...
  rpt->push =
      JS_NewCFunctionData(ctx, &js_repeater_push, 0, 0, 1, (JSValueConst*)&obj);
  rpt->stop =
      JS_NewCFunctionData(ctx, &js_repeater_stop, 0, 0, 1, (JSValueConst*)&obj);
...
static JSValue
js_repeater_constructor(JSContext* ctx,
...
static JSValue
js_repeater_next(JSContext* ctx,
                 JSValueConst this_val,
                 int argc,
                 JSValueConst argv[],
                 BOOL* pdone,
...
static JSValue
js_repeater_funcs(JSContext* ctx,
                  JSValueConst this_val,
                  int argc,
                  JSValueConst argv[],
                  int magic) {
...
js_repeater_init(JSContext* ctx, JSModuleDef* m) {
  ...
  repeater_ctor = JS_NewCFunction2(
      ctx, js_repeater_constructor, "Repeater", 1, JS_CFUNC_constructor, 0);
