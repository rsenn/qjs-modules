...
function Balancer(open, close, debug) {
  ...
  return function Balancer(tok) {
    ...
    if(close) stack.pop();
    if(debug && (open || close)) console.log('Balancer', { tok: InspectToken(tok), stack: stack.map(InspectToken) });
...
class EBNFParser extends Parser {
  constructor(grammar) {
    ...
    this.lexer.handler = (arg, tok) => {
      ...
      let error = new Error(`Unmatched token at ${arg.loc} char='${Lexer.escape(line[index])}' section=${this.section} state=${arg.states[arg.state]}\n${line}\n${[...line]
          .slice(0, index)
          .map(c => (c != '\t' ? ' ' : c))
          .join('')}^`
      );
