...
function main(...args) {
  ...
  const isKeyword = word =>
    ...
    const isPunctuator = word => /^(=|\.|-|%|}|>|,|\*|\[|<|!|\/|\]|~|\&|\(|;|\?|\||\)|:|\+|\^|{|@|!=|\*=|\&\&|<<|\/=|\|\||>>|\&=|==|\+\+|\|=|<=|--|\+=|\^=|>=|-=|%=|=>|\${|\?\.|\*\*|\?\?|!==|===|>>>|>>=|-->>|<<=|\.\.\.|\*\*=|\|\|=|\&\&=|\?\?=|>>>=|-->>=)$/.test(word);
  ...
  function lexIdentifier() {
    ...
    if(Lexer.isDecimalDigit(firstChar)) throw this.error(`Invalid IDENTIFIER`);
    const c = this.peek();
    if(c == '`') {
  ...
  function lexQuote() {
    ...
    do {
      if(this.acceptRun(
          Predicate.and(Predicate.not(Lexer.isLineTerminator),
