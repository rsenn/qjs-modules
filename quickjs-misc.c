...
set_text_attributes(intptr_t fd, uint32_t attr) {
  ...
  bg = ((attr & BACKGROUND_RED) ? 1 : 0) + ((attr & BACKGROUND_GREEN) ? 2 : 0) + ((attr & BACKGROUND_BLUE) ? 4 : 0) + ((attr & BACKGROUND_INTENSITY) ? 100 : 40);
...
js_arraybuffer_free_cstring(JSRuntime* rt, void* opaque, void* ptr) {
  JSValue value = JS_MKPTR(JS_TAG_STRING, (JSString*)(void*)((uint8_t*)opaque - offsetof(JSString, u)));
...
js_misc_slice(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  if((data = JS_GetArrayBuffer(ctx, &len, argv[0]))) {
    ...
    return JS_NewArrayBuffer(ctx, data + ir.start, ir.end - ir.start, js_arraybuffer_free_object, (void*)obj, js_is_sharedarraybuffer(ctx, argv[0]));
  }
...
js_misc_duparraybuffer(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  if((data = JS_GetArrayBuffer(ctx, &len, argv[0]))) {
    ...
    return JS_NewArrayBuffer(ctx, data + ol.offset, ol.length, js_arraybuffer_free_object, (void*)obj, js_is_sharedarraybuffer(ctx, argv[0]));
  }
...
js_misc_getx(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[], int magic) {
  ...
  if(ret == -1)
    return JS_ThrowInternalError(
        ctx,
        "%s() failed: %s",
        ((const char* const[]){"getpid", "getppid", "getsid", "getuid", "getgid", "geteuid", "getegid", "setuid", "setgid", "seteuid", "setegid"})[magic - FUNC_GETPID],
        strerror(errno));
