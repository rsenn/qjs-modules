...
#define JS_SOCKETCALL(syscall_index, socket, retval)                                                                   \
  JS_SOCKETCALL_RETURN(syscall_index, socket, retval, JS_NewInt32(ctx, socket.ret), JS_NewInt32(ctx, -1))

#define JS_SOCKETCALL_FAIL(syscall_index, socket, failval)                                                             \
  JS_SOCKETCALL_RETURN(syscall_index, socket, retval, JS_NewInt32(ctx, result), failval)
  ...
  switch(magic) {
    case SOCKET_PROP_MODE: {
      ...
      JS_SOCKETCALL_RETURN(
          SYSCALL_FCNTL, sock, fcntl(sock.fd, F_SETFL, mode), JS_NewInt32(ctx, sock.ret), JS_UNDEFINED);
      break;
    }
  }
  JS_SetOpaque(this_val, sock.ptr);
  ...
  if(socket_open(sock)) {
    switch(magic) {
      ...
      case SOCKET_METHOD_SENDTO: {
        ...
        if(magic == SOCKET_METHOD_SEND) {
          JS_SOCKETCALL(SYSCALL_SEND,
                        sock,
                        socket_send(sock.fd, buf.data + off.offset, offset_size(&off, buf.size), flags));
        } else {
          SockAddr* sa = js_sockaddr_data(argv[4]);
...
static const JSCFunctionListEntry js_socket_proto_funcs[] = {
    ...
    JS_CFUNC_MAGIC_DEF("sendto", 2, js_socket_method, SOCKET_METHOD_SENDTO),
    JS_CFUNC_MAGIC_DEF("recv", 1, js_socket_method, SOCKET_METHOD_RECV),
    JS_CFUNC_MAGIC_DEF("recvfrom", 2, js_socket_method, SOCKET_METHOD_RECVFROM),
