...
#define JS_SOCKETCALL(syscall_index, socket, retval)                           \
  JS_SOCKETCALL_RETURN(syscall_index,                                          \
                       socket,                                                 \
                       retval,                                                 \
                       JS_NewInt32(ctx, socket.ret),                           \
                       JS_NewInt32(ctx, -1))

#define JS_SOCKETCALL_FAIL(syscall_index, socket, failval)                     \
  JS_SOCKETCALL_RETURN(                                                        \
      syscall_index, socket, retval, JS_NewInt32(ctx, result), failval)

#define JS_SOCKETCALL_RETURN(                                                  \
    syscall_index, socket, retval, successval, failval)                        \
  do {                                                                         \
    socket.ret = retval;                                                       \
...
static const char* socket_syscalls[] = {0,
                                        "socket",
                                        "getsockname",
                                        "getpeername",
                                        "fcntl",
...
js_sockaddr_inspect(JSContext* ctx,
  ...
  if(buf[0])
    JS_DefinePropertyValueStr(
        ctx, obj, "addr", JS_NewString(ctx, buf), JS_PROP_ENUMERABLE);

  if((port = sockaddr_port(sa)))
    JS_DefinePropertyValueStr(
        ctx, obj, "port", JS_NewUint32(ctx, port), JS_PROP_ENUMERABLE);
...
static const JSCFunctionListEntry js_sockaddr_proto_funcs[] = {
    JS_CGETSET_MAGIC_DEF(
        "family", js_sockaddr_get, js_sockaddr_set, SOCKADDR_PROP_FAMILY),
    JS_CGETSET_MAGIC_DEF(
        "addr", js_sockaddr_get, js_sockaddr_set, SOCKADDR_PROP_ADDR),
    JS_CGETSET_MAGIC_DEF(
        "port", js_sockaddr_get, js_sockaddr_set, SOCKADDR_PROP_PORT),
    JS_CFUNC_MAGIC_DEF("clone", 0, js_sockaddr_method, SOCKADDR_METHOD_CLONE),
    JS_CFUNC_MAGIC_DEF(
        "toString", 0, js_sockaddr_method, SOCKADDR_METHOD_TOSTRING),
...
js_poll(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  int32_t timeout = -1;
  struct pollfd* pfds;
  BOOL is_array = js_is_array(ctx, argv[0]),
       is_arraybuffer = js_is_arraybuffer(ctx, argv[0]);
  ...
  switch(magic) {
    ...
    case SOCKET_PROP_ERROR: {
      ...
      if(sock.error) {
        ...
        JS_SetPropertyStr(ctx,
                          ret,
                          "message",
                          JS_NewString(ctx, strerror(sock.error)));
    ...
    case SOCKET_PROP_LOCAL: {
      ...
      JS_SOCKETCALL_RETURN(SYSCALL_GETSOCKNAME,
                           sock,
                           getsockname(sock.fd, (struct sockaddr*)sa, &len),
    ...
    case SOCKET_PROP_REMOTE: {
      ...
      JS_SOCKETCALL_RETURN(SYSCALL_GETPEERNAME,
                           sock,
                           getpeername(sock.fd, (struct sockaddr*)sa, &len),
                           js_sockaddr_wrap(ctx, sa),
                           JS_NULL);
      break;
    }
  ...
  switch(magic) {
    case SOCKET_PROP_MODE: {
      ...
      JS_SOCKETCALL_RETURN(SYSCALL_FCNTL,
                           sock,
                           fcntl(sock.fd, F_SETFL, mode),
                           JS_NewInt32(ctx, sock.ret),
                           JS_UNDEFINED);
      break;
    }
  }
  ...
  if(socket_open(sock)) {
    switch(magic) {
      ...
      case SOCKET_METHOD_BIND: {
        ...
        JS_SOCKETCALL(SYSCALL_BIND,
                      sock,
                      bind(sock.fd, (struct sockaddr*)sa, sockaddr_size(sa)));
        break;
      }
      case SOCKET_METHOD_ACCEPT: {
        ...
        if(!(sa = js_sockaddr_data(argv[0])))
          return JS_ThrowTypeError(ctx, "argument 1 must be of type SockAddr");
        JS_SOCKETCALL(SYSCALL_ACCEPT,
                      sock,
                      accept(sock.fd, (struct sockaddr*)sa, &addrlen));
        break;
      }
      ...
      case SOCKET_METHOD_RECV: {
        ...
        // off.offset, off.length, offset_data(&off, buf.data, buf.size),
        // offset_size(&off, buf.data, buf.size));
      ...
      case SOCKET_METHOD_GETSOCKOPT: {
        ...
        JS_SOCKETCALL(SYSCALL_GETSOCKOPT,
                      sock,
                      getsockopt(sock.fd, level, optname, buf, &len));
        if(tmp) {
          js_array_clear(ctx, argv[2]);
          JS_SetPropertyUint32(ctx,
                               argv[2],
                               0,
                               JS_NewInt32(ctx, *(int32_t*)buf));
        }
        val = (void*)buf;
        if(tmp)
          js_free(ctx, tmp);
      ...
      case SOCKET_METHOD_SETSOCKOPT: {
        ...
        JS_SOCKETCALL(SYSCALL_SETSOCKOPT,
                      sock,
                      setsockopt(sock.fd, level, optname, buf, len));
...
js_socket_inspect(JSContext* ctx,
  ...
  JS_DefinePropertyValueStr(
      ctx, obj, "fd", JS_NewUint32(ctx, sock.fd), JS_PROP_ENUMERABLE);
  if(sock.ret >= 0) {
    JS_DefinePropertyValueStr(
        ctx, obj, "ret", JS_NewUint32(ctx, sock.ret), JS_PROP_ENUMERABLE);
  } else {
    const char* syscall = socket_syscall(sock);
    JS_DefinePropertyValueStr(
        ctx, obj, "errno", JS_NewUint32(ctx, sock.error), JS_PROP_ENUMERABLE);
  ...
  if(sock.syscall > 0 && sock.syscall < socket_syscalls_size)
    JS_DefinePropertyValueStr(ctx,
                              obj,
                              "syscall",
                              JS_NewString(ctx, socket_syscall(sock)),
                              JS_PROP_ENUMERABLE);
...
static JSValue
js_socket_valueof(JSContext* ctx,
                  JSValueConst this_val,
                  int argc,
                  JSValueConst argv[]) {
...
static const JSCFunctionListEntry js_socket_proto_funcs[] = {
    JS_CGETSET_MAGIC_FLAGS_DEF(
        "fd", js_socket_get, 0, SOCKET_PROP_FD, JS_PROP_C_W_E),
    ...
    JS_CGETSET_MAGIC_DEF(
        "mode", js_socket_get, js_socket_set, SOCKET_PROP_MODE),
    ...
    JS_CFUNC_MAGIC_DEF(
        "getsockopt", 3, js_socket_method, SOCKET_METHOD_GETSOCKOPT),
    JS_CFUNC_MAGIC_DEF(
        "setsockopt", 3, js_socket_method, SOCKET_METHOD_SETSOCKOPT),
...
js_sockets_init(JSContext* ctx, JSModuleDef* m) {
  ...
  JSValue fdset_module, fdset_ns, fdset_ctor = JS_UNDEFINED, socklen_module,
                                  socklen_ns, socklen_ctor = JS_UNDEFINED;
