...
BOOL
pointer_truncate(Pointer* ptr,  int64_t newsz, JSContext* ctx) {
  Pointer* ret = 0;
  size_t i, del, len, newlen;

  newsz = newsz < 0 ?  int64_mod(newsz, ptr->n + 1) : MAX_NUM(newsz, ptr->n);
...
#ifdef DEBUG_OUTPUT
  printf("%s() newsz: %li end: %li del: %li ins: %lu len: %lu newlen: %lu remain: %li\n",
         __func__,
         (long)newsz,
         (long)end,
         (long)del,
  ...
  if(del) {
    ...
    memcpy(ret->atoms, &ptr->atoms[newsz], del * sizeof(JSAtom));
  }
  ...
  if(ins)
    memcpy(&ptr->atoms[newsz], atoms, ins * sizeof(JSAtom));
