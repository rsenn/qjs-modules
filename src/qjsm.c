...
jsm_module_loader(JSContext* ctx, const char* module_name, void* opaque) {
  ...
  char* jsm_module_normalize(JSContext * ctx, const char* path, const char* name, void* opaque) {
    size_t p;
    const char* r;
    DynBuf file = {0, 0, 0};
    size_t n;
  ...
  static JSValue jsm_eval_file(JSContext * ctx, const char* file, BOOL module) {
    uint8_t* buf;
    size_t len;
    int flags;
  ...
  static JSValue jsm_script_get(JSContext * ctx, JSValueConst this_val, int magic) {
  ...
  static void jsm_script_push(JSContext * ctx, const char* file) { vector_putptr(&jsm_scripts, js_strdup(ctx, file)); }

  static void jsm_script_pop(JSContext * ctx) {
    char** ptr;
  ...
  static int jsm_script_load(JSContext * ctx, const char* file, BOOL module) {
    JSValue val;
    int32_t ret = 0;
  ...
  static JSContext* jsm_context_new(JSRuntime * rt) {
    JSContext* ctx;
...
#define MALLOC_OVERHEAD 8
  ...
  static void dump_vector(const Vector* vec, size_t start) {
  ...
  static inline unsigned long long jsm_trace_malloc_ptr_offset(uint8_t * ptr, struct trace_malloc_data * dp) { return ptr - dp->base; }

  /* default memory allocation functions with memory limitation */
  static inline size_t jsm_trace_malloc_usable_size(void* ptr) {
#if defined(__APPLE__)
...
#warning change this to `return 0;` if compilation fails
  ...
  static void jsm_trace_malloc_init(struct trace_malloc_data * s) { free(s->base = malloc(8)); }

  static void* jsm_trace_malloc(JSMallocState * s, size_t size) {
    void* ptr;
  ...
  static void jsm_trace_free(JSMallocState * s, void* ptr) {
    if(!ptr)
      return;
  ...
  static void* jsm_trace_realloc(JSMallocState * s, void* ptr, size_t size) {
    size_t old_size;
...
#define PROG_NAME "qjsm"
  ...
  static JSValue jsm_eval_script(JSContext * ctx, JSValueConst this_val, int argc, JSValueConst argv[], int magic) {
    const char* str;
    size_t len;
    JSValue ret;
    int32_t module;
  ...
  static JSValue jsm_module_func(JSContext * ctx, JSValueConst this_val, int argc, JSValueConst argv[], int magic) {
  ...
  void jsm_import_parse(ImportDirective * imp, const char* spec) {
    BOOL ns;
