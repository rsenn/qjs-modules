...
BOOL
js_mysql_nonblock(JSContext* ctx, JSValueConst value) {
...
static void
js_value_escape(JSContext* ctx, DynBuf* out, JSValueConst value) {
  ...
  } else if(!JS_IsBigDecimal(/*ctx,*/ value) && js_is_numeric(ctx, value)) {
    JSValue val = js_value_coerce(ctx, (JS_IsBigDecimal(value)) ? "Number" : "BigDecimal", value);

    js_value_escape(ctx, out, val);
    JS_FreeValue(ctx, val);
...
void
js_mysql_connect_params(JSContext* ctx, struct MySQLConnectParms* c, int argc, JSValueConst argv[]) {
...
void
js_mysql_connectparms_free(JSContext* ctx, struct MySQLConnectParms* c) {
...
js_mysql_connect_start(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  struct MySQLConnectParms c = {0, 0, 0, 0, 0};
  MYSQL *my, *ret = 0;
  ...
  js_mysql_connect_params(ctx, &c, argc, argv);

  status = mysql_real_connect_start(&ret, my, c.host, c.user, c.password, c.db, c.port, c.socket, c.flags);
  ...
  js_mysql_connectparms_free(ctx, &c);
...
static const JSCFunctionListEntry js_mysql_defs[] = {
    ...
    JS_PROP_INT64_DEF("COUNT_ERROR", MYSQL_COUNT_ERROR, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("WAIT_READ", MYSQL_WAIT_READ, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("WAIT_WRITE", MYSQL_WAIT_WRITE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("WAIT_EXCEPT", MYSQL_WAIT_EXCEPT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("WAIT_TIMEOUT", MYSQL_WAIT_TIMEOUT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("DATABASE_DRIVER", MYSQL_DATABASE_DRIVER, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("DEFAULT_AUTH", MYSQL_DEFAULT_AUTH, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("ENABLE_CLEARTEXT_PLUGIN", MYSQL_ENABLE_CLEARTEXT_PLUGIN, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("INIT_COMMAND", MYSQL_INIT_COMMAND, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_BIND", MYSQL_OPT_BIND, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CAN_HANDLE_EXPIRED_PASSWORDS", MYSQL_OPT_CAN_HANDLE_EXPIRED_PASSWORDS, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_COMPRESS", MYSQL_OPT_COMPRESS, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CONNECT_ATTR_ADD", MYSQL_OPT_CONNECT_ATTR_ADD, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CONNECT_ATTR_DELETE", MYSQL_OPT_CONNECT_ATTR_DELETE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CONNECT_ATTR_RESET", MYSQL_OPT_CONNECT_ATTR_RESET, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CONNECT_ATTRS", MYSQL_OPT_CONNECT_ATTRS, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_CONNECT_TIMEOUT", MYSQL_OPT_CONNECT_TIMEOUT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_GUESS_CONNECTION", MYSQL_OPT_GUESS_CONNECTION, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_LOCAL_INFILE", MYSQL_OPT_LOCAL_INFILE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_MAX_ALLOWED_PACKET", MYSQL_OPT_MAX_ALLOWED_PACKET, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_NAMED_PIPE", MYSQL_OPT_NAMED_PIPE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_NET_BUFFER_LENGTH", MYSQL_OPT_NET_BUFFER_LENGTH, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_NONBLOCK", MYSQL_OPT_NONBLOCK, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_PROTOCOL", MYSQL_OPT_PROTOCOL, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_READ_TIMEOUT", MYSQL_OPT_READ_TIMEOUT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_RECONNECT", MYSQL_OPT_RECONNECT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CA", MYSQL_OPT_SSL_CA, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CAPATH", MYSQL_OPT_SSL_CAPATH, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CERT", MYSQL_OPT_SSL_CERT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CIPHER", MYSQL_OPT_SSL_CIPHER, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CRL", MYSQL_OPT_SSL_CRL, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_CRLPATH", MYSQL_OPT_SSL_CRLPATH, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_ENFORCE", MYSQL_OPT_SSL_ENFORCE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_KEY", MYSQL_OPT_SSL_KEY, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_SSL_VERIFY_SERVER_CERT", MYSQL_OPT_SSL_VERIFY_SERVER_CERT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_TLS_VERSION", MYSQL_OPT_TLS_VERSION, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_USE_EMBEDDED_CONNECTION", MYSQL_OPT_USE_EMBEDDED_CONNECTION, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_USE_REMOTE_CONNECTION", MYSQL_OPT_USE_REMOTE_CONNECTION, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_USE_RESULT", MYSQL_OPT_USE_RESULT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("OPT_WRITE_TIMEOUT", MYSQL_OPT_WRITE_TIMEOUT, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("PLUGIN_DIR", MYSQL_PLUGIN_DIR, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("PROGRESS_CALLBACK", MYSQL_PROGRESS_CALLBACK, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("READ_DEFAULT_FILE", MYSQL_READ_DEFAULT_FILE, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("READ_DEFAULT_GROUP", MYSQL_READ_DEFAULT_GROUP, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("REPORT_DATA_TRUNCATION", MYSQL_REPORT_DATA_TRUNCATION, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SECURE_AUTH", MYSQL_SECURE_AUTH, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SERVER_PUBLIC_KEY", MYSQL_SERVER_PUBLIC_KEY, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SET_CHARSET_DIR", MYSQL_SET_CHARSET_DIR, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SET_CHARSET_NAME", MYSQL_SET_CHARSET_NAME, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SET_CLIENT_IP", MYSQL_SET_CLIENT_IP, JS_PROP_C_W_E),
    JS_PROP_INT32_DEF("SHARED_MEMORY_BASE_NAME", MYSQL_SHARED_MEMORY_BASE_NAME, JS_PROP_C_W_E),
};
...
result_yield(JSContext* ctx, JSValueConst func, MYSQL_RES* res, MYSQL_ROW row, int rtype) {
  JSValue result = result_row(ctx, res, row, rtype);
...
/*static BOOL
result_nonblock(MYSQL_RES* res) {
  return mysql_nonblock(res->handle);
}*/
...
static inline int
js_mysqlresult_type(JSContext* ctx, JSValueConst value) {
  JSValue handle;
  int rtype;
  handle = JS_GetPropertyStr(ctx, value, "handle");
  rtype = js_get_propertystr_int32(ctx, handle, "resultType");
  JS_FreeValue(ctx, handle);
  return rtype;
}
...
js_mysqlresult_next_cont(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[], int magic, JSValue data[]) {
  ...
  if(status == 0 && num_fields == field_count) {
    ...
    result_yield(ctx, data[3], res, row, js_mysqlresult_type(ctx, data[1]));
...
js_mysqlresult_next_start(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  if(status == 0 && num_fields == field_count) {
    result_yield(ctx, data[3], res, row, js_mysqlresult_type(ctx, this_val));
...
js_mysqlresult_next(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst argv[]) {
  ...
  ret = result_row(ctx, res, row, js_mysqlresult_type(ctx, this_val));
  return ret;
}
...
js_mysql_init(JSContext* ctx, JSModuleDef* m) {
  if(js_mysql_class_id == 0) {
    ...
    JS_SetPropertyFunctionList(ctx, mysql_proto, js_mysql_defs, countof(js_mysql_defs));
    JS_SetPropertyFunctionList(ctx, mysql_ctor, js_mysql_static_funcs, countof(js_mysql_static_funcs));
    JS_SetPropertyFunctionList(ctx, mysql_ctor, js_mysql_defs, countof(js_mysql_defs));
